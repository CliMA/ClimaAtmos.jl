...
output_dir = joinpath(job_id, "active_output")
mkpath(output_dir)
...
allocs_limit = Dict()
allocs_limit["sphere_baroclinic_wave_rhoe"] = 226528
allocs_limit["diagnostic_edmfx_aquaplanet"] = 4092624
allocs_limit["sphere_baroclinic_wave_rhoe_equilmoist_expvdiff"] = 335360
allocs_limit["gpu_aquaplanet_dyamond_ss_1process"] = 4_015_547_056
# allocs_limit["flame_perf_target_threaded"] = 1_276_864
# allocs_limit["flame_perf_target_callbacks"] = 988_816
allocs_limit["single_column_nonorographic_gravity_wave"] = 436152
allocs_limit["gpu_prognostic_edmfx_aquaplanet"] = 4_000_488
allocs_limit["sphere_baroclinic_wave_rhoe_equilmoist"] = 336_378
...
expected_allocs = get(allocs_limit, config_id, max_allocs_for_full_sampling)
...
@info "`allocs ($config_id)`: $(allocs)"

if allocs < allocs_limit[config_id] * buffer
    @info "TODO: lower `allocs_limit[$config_id]` to: $(allocs)"
end
Δallocs = allocs / allocs_limit[config_id]
...
@testset "Allocations limit" begin
    if device isa ClimaComms.CUDADevice # https://github.com/CliMA/ClimaAtmos.jl/issues/2831
        @test allocs ≤ allocs_limit[config_id] * buffer
    else
        @test 0.25 * allocs_limit[config_id] * buffer <=
              allocs ≤
              allocs_limit[config_id] * buffer
    end
end
