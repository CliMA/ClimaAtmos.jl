---
name: scm-les-tuning
description: Sweep ClimaAtmos single-column (SCM) EDMF/microphysics toml parameters to match path-integrated quantities (IWP, SWP, LWP, RWP, cloud top) against LES reference data such as Stats.TRMM_LBA.nc or Stats.DYCOMS_RF01.nc, OR against ARM SGP observations for the ARM VARANAL column config. Use when tuning microphysics or EDMF parameters for a config/model_configs/*_column.yml run against an LES Stats.*.nc file or ARM obs, or when asked to "match LES", "tune against LES", "match observations", or investigate why a column run produces zero/wrong IWP-SWP-LWP-RWP.
---
...
## ARM VARANAL case (obs tuning)

The ARM VARANAL config compares against real ARM SGP observations rather than
LES. The mechanics (change toml → run → compare → checkpoint) are the same,
but the comparison backend is different.

**Base files:**
- Config: `config/model_configs/prognostic_edmfx_armvaranal_column.yml`
- Toml: `toml/prognostic_edmfx_varanal_sgp.toml` (already has `detr_massflux_vertdiv_coeff=0.1`)
- Run duration: `t_end: "4days"` (~40s per run at 45 levels)

**Sweep runs** follow the same pattern as LES (one toml copy + override yml per run):

```bash
python3 .claude/skills/scm-les-tuning/scripts/set_toml_value.py \
  toml/prognostic_edmfx_varanal_sgp.toml /tmp/arm_sweep/run1.toml \
  precipitation_timescale=1500 entr_coeff=0.08
```

```bash
cat > /tmp/arm_sweep/run1.yml <<EOF
toml: ["/tmp/arm_sweep/run1.toml"]
job_id: "arm_sweep_run1"
output_dir: "/tmp/arm_sweep/output_run1"
output_dir_style: "RemovePreexisting"
EOF
```
...
julia --project=.buildkite .buildkite/ci_driver.jl \
  ...
  --config_file /tmp/arm_sweep/run1.yml \
  > /tmp/arm_sweep/run1.log 2>&1
echo "EXIT $?"
```

Extract and compare (use `compare_to_arm_obs.jl`, not `compare_to_les.jl`):

```bash
mkdir -p /tmp/arm_sweep/extracted_run1
tar -xf /tmp/arm_sweep/output_run1/nc_files.tar -C /tmp/arm_sweep/extracted_run1

julia --project=.buildkite .claude/skills/scm-les-tuning/scripts/compare_to_arm_obs.jl \
  /tmp/arm_sweep 20100918 baseline=extracted_baseline run1=extracted_run1
```

The script prints a table of biases (model − obs) for LWP, cloud fraction,
precip, PWV, SW↓, LW↓. A second column shows LWP RMSE (time-matched).

### Deep-dive PDFs (selected runs)

For a run worth examining in detail, run the full CI plot pipeline to generate
`obs_comparison.pdf` (sonde profiles, CLDRAD cloud fraction, radiation, fluxes):

```bash
julia --project=.buildkite -e '
  using ClimaAtmos, ClimaAnalysis
  import ClimaAtmos.Diagnostics.ci_plots: make_plots
  make_plots(Val(:prognostic_edmfx_armvaranal_column), ["/tmp/arm_sweep/output_run1"])
'
```

This produces `obs_comparison.pdf` and `obs_comparison_statistics.txt` in the
output directory. Use this for the final "best" run and for diagnosing *why*
a metric is off, not in the inner sweep loop (it takes ~30s to generate).

### ARM obs reference for 20100918 (4-day period, SGP)

These are the observational means the model is compared against:

| Variable | Obs mean | Source |
|----------|----------|--------|
| LWP | ~5 g/m² | CLDRAD MWR |
| Cloud fraction | ~21% | CLDRAD ARSCL |
| PWV | ~36 mm (3.6 cm) | CLDRAD |
| SW↓ (surface) | ~220 W/m² | CLDRAD |
| LW↓ (surface) | ~389 W/m² | CLDRAD |
| Precip | ~0 mm/hr | BEATM (no rain this period) |

**This is a mostly clear-sky, fair-weather period** — very different from
TRMM-LBA. LWP of 5 g/m² is thin boundary-layer cloud or patchy fair-weather
Cu; the model's task is not to produce deep convection but to avoid
overestimating cloud and LWP.

### Unit gotchas

- Obs `pwv` is in **cm**; the script multiplies by 10 to compare with model
  `prw` (kg/m² = mm).
- Obs `tot_cld` is 0–1; the script multiplies by 100 to compare with model
  `clt` (%).
- Model `pr` is negative (sign convention: downward fluxes negative); the
  script negates and scales by 3600 to get mm/hr.

### ARM VARANAL-specific toml parameters

Unlike TRMM-LBA, the ARM toml includes relaxation parameters that nudge the
column back towards the large-scale state above the PBL:

| Parameter | Default | Effect |
|-----------|---------|--------|
| `gcmdriven_momentum_relaxation_timescale` | 21600 s (6h) | How tightly horizontal wind is nudged above `gcmdriven_relaxation_minimum_height` |
| `gcmdriven_scalar_relaxation_timescale` | 86400 s (24h) | Same for scalars (T, q) |
| `gcmdriven_relaxation_minimum_height` | 5500 m | Bottom of nudging layer |
| `gcmdriven_relaxation_maximum_height` | 8000 m | Top of nudging layer (full nudge above) |

Tightening the scalar relaxation timescale (e.g. 43200 s) brings T/q profiles
closer to the forcing data but removes degrees of freedom from PBL evolution.
Loosening it (e.g. 172800 s) lets the column drift further from the forcing.
Don't touch these unless diagnosing a mean-state bias in the sonde profiles;
the default values were set for this specific ARM SGP forcing.
