...
`"plane"`, and for `"multicolumn"` that `column_latitudes` and `column_longitudes`
are non-empty and of equal length; that an ISDAC run (`initial_condition: ISDAC`)
uses a moist microphysics model; that implicit vertical diffusion is paired with a
turbulence-convection model, a vertical diffusion model, or a vertically-acting
Smagorinsky-Lilly closure; and that prescribed flow is used only with flat
topography and an explicit solver. Each check is independent. Called at the top
of `get_atmos`.
"""
