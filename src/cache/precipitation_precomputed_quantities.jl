...
import ClimaCore.MatrixFields: get_field
...
internal_energy_func(qᵪ_name) =
    qᵪ_name == @name(q_liq) ? Iₗ :
    qᵪ_name == @name(q_ice) ? Iᵢ :
    qᵪ_name == @name(q_rai) ? Iₗ :
    qᵪ_name == @name(q_sno) ? Iᵢ : error("Invalid tracer name $qᵪ_name")

ρξw_div_ρξ_bounded(ρqw, ρq) = ifelse(ρq > zero(ρq), ρqw / ρq, zero(ρq))

function total_energy_func(qᵪ_name)
    I = internal_energy_func(qᵪ_name)
    total_energy(thp, ᶜts, ᶜΦ, ᶜwᵪ, ᶜu_uvw) = I(thp, ᶜts) + ᶜΦ + Kin(ᶜwᵪ, ᶜu_uvw)
end
function internal_plus_geopotential_energy_func(qᵪ_name)
    I = internal_energy_func(qᵪ_name)
...
function set_precipitation_velocities!(Y, p,
    ...
    for qᵪ_name in qᵪ_names
        ...
        wᵪ_func = terminal_velocity_func_1M(cmc, cmp, qᵪ_name)  # terminal velocity for tracer `q`
        ρaqʲ(ᶜsgsʲ) = begin
            # Calculate `ρa ⋅ qᵪ` for subdomain `j`
            qᵪʲ = get_field(ᶜsgsʲ, qᵪ_name)
            ᶜsgsʲ.ρa * qᵪʲ
        end
        ρaqwʲ(ᶜsgsʲ, ᶜρʲ) = begin
            # Calculate `ρa ⋅ qᵪ ⋅ wᵪ` for subdomain `j`
            qᵪʲ = get_field(ᶜsgsʲ, qᵪ_name)
            wᵪʲ = wᵪ_func(ᶜρʲ, clip(qᵪʲ))
            ᶜsgsʲ.ρa * qᵪʲ * wᵪʲ
        end
        calc_hₜ = internal_plus_geopotential_energy_func(qᵪ_name)  # TODO: do we need to add kinetic energy of subdomains?
        ...
        # TODO: use Y.c.ρq instead of ᶜρqᵪ for computing gs velocities by averaging velocities 
        #       over subdomains once negative subdomain mass issues are resolved
        clip_ρaqʲ(ᶜsgsʲ) = clip(ρaqʲ(ᶜsgsʲ))
        ᶜρqᵪ = @. lazy(clip(ᶜρa⁰ * ᶜqᵪ⁰) + draft_sum(clip_ρaqʲ, ᶜsgsʲs))

        ᶜwᵪ⁰ = @. lazy(wᵪ_func(ᶜρ⁰, clip(ᶜqᵪ⁰)))  # environment terminal velocity
        ᶜρaqwᵪ⁰ = @. lazy(ᶜρa⁰ * ᶜqᵪ⁰ * ᶜwᵪ⁰)  # environment ρa ⋅ qᵪ ⋅ wᵪ
...
function set_precipitation_velocities!(Y, p,
    ...
    for qᵪ_name in qᵪ_names
        ...
        calc_hₜ = internal_plus_geopotential_energy_func(qᵪ_name)
        # Get grid-scale quantities
        ᶜqᵪ = ᶜspecific_gs_tracer(Y, qᵪ_name)
        ᶜwᵪ = get_field(p.precomputed, get_ᶜwᵪ_name_from_qᵪ_name(qᵪ_name))
        ᶜwₙᵪ = get_field(p.precomputed, get_ᶜwₙᵪ_name_from_qᵪ_name(qᵪ_name))
