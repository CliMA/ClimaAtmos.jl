...
function compute_covariance(Y, p, thermo_params, ᶜts)
    ...
    ᶜq′q′ = @. lazy(cov_from_grad(coeff, ᶜmixing_length_field, WV(ᶜgradᵥ_q_tot), WV(ᶜgradᵥ_q_tot)))
    ᶜθ′θ′ = @. lazy(cov_from_grad(coeff, ᶜmixing_length_field, WV(ᶜgradᵥ_θ_liq_ice), WV(ᶜgradᵥ_θ_liq_ice)))
    ᶜθ′q′ = @. lazy(cov_from_grad(coeff, ᶜmixing_length_field, WV(ᶜgradᵥ_θ_liq_ice), WV(ᶜgradᵥ_q_tot)))
