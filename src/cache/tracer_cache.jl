...
function tracer_cache(Y, prescribed_aerosol_names, time_varying_trace_gases, start_date)
    ...
    if :o3 in Symbol.(lowercase.(time_varying_trace_gases))
        o3_cache = ozone_cache(Y, start_date)
    else
        o3_cache = (;)
    end

    if :co2 in Symbol.(lowercase.(time_varying_trace_gases))
        co2_cache = co2_cache(Y, start_date)
    else
        co2_cache = (;)
    end

    return (; aerosol_cache..., o3_cache..., co2_cache...)
end
