struct AtmosCache{
    ...
    """2D surface ocean fraction field, populated by the coupler before the first timestep"""
...
function build_cache(
    ...
    tracers = tracer_cache(
        ...
        time_varying_trace_gas_names,
        start_date,
        _aerosol_names(atmos.interactive_aerosols),
    )
    ...
    # prescribed (climatology) or interactive (state-variable) path, so pass the
    # union of both name lists to the radiation cache gate.
    # TODO: interactive aerosol mass (Y.c.ρ<name>) is not yet fed to RRTMGP --
    # update_prescribed_aerosol_concentrations! reads only prescribed_aerosols_field,
    # so an interactive-only bin passes this gate but currently contributes 0 to
    # radiation. Remove interactive names from this union once the radiation feed
    # reads interactive aerosols. See update_inputs.jl.
    radiation_aerosol_names = (
        prescribed_aerosol_names...,
        string.(_aerosol_names(atmos.interactive_aerosols))...,
    )
    radiation_args =
