    ...
    if moisture_model isa NonEquilMoistModel &&
       ...
       microphysics_model isa Microphysics2Moment
        ...
        if microphysics_model isa Microphysics2Moment
            (; ᶜn_liqʲs, ᶜn_raiʲs) = p.precomputed

            ᶜn_liq = @. lazy(specific(Y.c.ρn_liq, Y.c.ρ))
            ᶜn_rai = @. lazy(specific(Y.c.ρn_rai, Y.c.ρ))

            n_liq_int_level = Fields.field_values(Fields.level(ᶜn_liq, 1))
            n_rai_int_level = Fields.field_values(Fields.level(ᶜn_rai, 1))

            # TODO consider adding them to p.precomputed.sfc_conditions
            # Though they will always be zero.
            ρ_flux_n_liq_sfc_halflevel = FT(0)
            ρ_flux_n_rai_sfc_halflevel = FT(0)
        end
    end
        ...
        if moisture_model isa NonEquilMoistModel &&
           ...
           microphysics_model isa Microphysics2Moment
            ...
            if microphysics_model isa Microphysics2Moment
                ...
                n_liqʲ_int_level = Fields.field_values(Fields.level(ᶜn_liqʲ, 1))
                n_raiʲ_int_level = Fields.field_values(Fields.level(ᶜn_raiʲ, 1))

                @. n_liqʲ_int_level = sgs_scalar_first_interior_bc(
                    z_int_level - z_sfc_halflevel,
                    ρ_int_level,
                    FT(turbconv_params.surface_area),
                    n_liq_int_level,
                    buoyancy_flux_sfc_halflevel,
                    ρ_flux_n_liq_sfc_halflevel,
                    ustar_sfc_halflevel,
                    obukhov_length_sfc_halflevel,
                    local_geometry_int_halflevel,
                )
                @. n_raiʲ_int_level = sgs_scalar_first_interior_bc(
                    z_int_level - z_sfc_halflevel,
                    ρ_int_level,
                    FT(turbconv_params.surface_area),
                    n_rai_int_level,
                    buoyancy_flux_sfc_halflevel,
                    ρ_flux_n_rai_sfc_halflevel,
                    ustar_sfc_halflevel,
                    obukhov_length_sfc_halflevel,
                    local_geometry_int_halflevel,
                )
...
NVTX.@annotate function set_diagnostic_edmf_precomputed_quantities_do_integral!(
    ...
    microphysics_2m_params = CAP.microphysics_2m_params(params)
    cloud_params = CAP.microphysics_cloud_params(params)
    turbconv_params = CAP.turbconv_params(params)
    aerosol_params = cloud_params.aml
    ...
    for i in 2:Spaces.nlevels(axes(Y.c))
        ...
        if moisture_model isa NonEquilMoistModel &&
           ...
           microphysics_model isa Microphysics2Moment
            ...
            if microphysics_model isa Microphysics2Moment
                n_liq_level = Fields.field_values(Fields.level(ᶜn_liq, i))
                n_liq_prev_level =
                    Fields.field_values(Fields.level(ᶜn_liq, i - 1))

                n_rai_level = Fields.field_values(Fields.level(ᶜn_rai, i))
                n_rai_prev_level =
                    Fields.field_values(Fields.level(ᶜn_rai, i - 1))
            end
        end
        ...
        for j in 1:n
            ...
            if moisture_model isa NonEquilMoistModel &&
               ...
               microphysics_model isa Microphysics2Moment
                ...
                ᶜSᵖ_2 = p.scratch.ᶜtemp_scalar_2
                if microphysics_model isa Microphysics2Moment
                    ᶜn_liqʲ = ᶜn_liqʲs.:($j)
                    ᶜn_raiʲ = ᶜn_raiʲs.:($j)

                    ᶜS_n_liqʲ = p.precomputed.ᶜSnₗᵖʲs.:($j)
                    ᶜS_n_raiʲ = p.precomputed.ᶜSnᵣᵖʲs.:($j)
                end
            end
            ...
            if moisture_model isa NonEquilMoistModel &&
               ...
               microphysics_model isa Microphysics2Moment
                ...
                Sᵖ_2_prev_level =
                    Fields.field_values(Fields.level(ᶜSᵖ_2, i - 1))

                if microphysics_model isa Microphysics2Moment
                    n_liqʲ_level = Fields.field_values(Fields.level(ᶜn_liqʲ, i))
                    n_raiʲ_level = Fields.field_values(Fields.level(ᶜn_raiʲ, i))

                    n_liqʲ_prev_level =
                        Fields.field_values(Fields.level(ᶜn_liqʲ, i - 1))
                    n_raiʲ_prev_level =
                        Fields.field_values(Fields.level(ᶜn_raiʲ, i - 1))

                    S_n_liqʲ_prev_level =
                        Fields.field_values(Fields.level(ᶜS_n_liqʲ, i - 1))
                    S_n_raiʲ_prev_level =
                        Fields.field_values(Fields.level(ᶜS_n_raiʲ, i - 1))
                end
            end
            ...
            elseif moisture_model isa NonEquilMoistModel &&
                ...
                # Snow not implemented
                @. S_q_iceʲ_prev_level = FT(0)
                @. S_q_snoʲ_prev_level = FT(0)

                # Cloud sources from the updrafts
                @. S_q_liqʲ_prev_level += cloud_sources(
                    cloud_params.liquid,
                    thermo_params,
                    q_totʲ_prev_level,
                ...
                seasalt_num_prev_level = aerosol_params.c₀_seasalt
                seasalt_mean_radius_prev_level = aerosol_params.α_seasalt
                sulfate_num_prev_level = FT(0)

                @. S_n_liqʲ_prev_level += aerosol_activation_sources(
                    seasalt_num_prev_level,
                    seasalt_mean_radius_prev_level,
                    sulfate_num_prev_level,
                    q_totʲ_prev_level,
                    q_liqʲ_prev_level + q_raiʲ_prev_level,
                    q_iceʲ_prev_level + q_snoʲ_prev_level,
                    n_liqʲ_prev_level + n_raiʲ_prev_level,
                    ρʲ_prev_level,
                    max(
                        0,
                        w_component.(
            ...
            if microphysics_model isa Microphysics1Moment ||
               ...
               moisture_model isa NonEquilMoistModel
                ...
                ###
                ### Additional 2-moment microphysics tracers
                ###
                if microphysics_model isa Microphysics2Moment
                    # Cloud droplets
                    @. ρaʲu³ʲ_dataq_ =
            ...
            if i > 2
                ...
                if microphysics_model isa Microphysics1Moment ||
                   ...
                   moisture_model isa NonEquilMoistModel
                    ...
                    if microphysics_model isa Microphysics2Moment
                        @. n_liqʲ_level =
                            ifelse(kill_updraft_2, n_liq_level, n_liqʲ_level)
                        @. n_raiʲ_level =
                            ifelse(kill_updraft_2, n_rai_level, n_raiʲ_level)
                    end
                end
            end
        ...
        if microphysics_model isa Microphysics1Moment ||
           ...
           moisture_model isa NonEquilMoistModel
            ...
            if microphysics_model isa Microphysics2Moment
                ...
                ᶜS_n_raiʲ = p.precomputed.ᶜSnᵣᵖʲs.:($j)
                S_n_liqʲ_level =
                    Fields.field_values(Fields.level(ᶜS_n_liqʲ, i_top))
                S_n_raiʲ_level =
                    Fields.field_values(Fields.level(ᶜS_n_raiʲ, i_top))
                @. S_n_liqʲ_level = 0
                @. S_n_raiʲ_level = 0
            end
        end
...
NVTX.@annotate function set_diagnostic_edmf_precomputed_quantities_env_precipitation!(
    ...
    # Rain sinks from the environment
    compute_precipitation_sinks!(
        ᶜSᵖ,
        ᶜSqᵣᵖ⁰,
        ᶜSqₛᵖ⁰,
...
NVTX.@annotate function set_diagnostic_edmf_precomputed_quantities_env_precipitation!(
    ...
    thermo_params = CAP.thermodynamics_params(p.params)
    microphysics_2m_params = CAP.microphysics_2m_params(p.params)
    cloud_params = CAP.microphysics_cloud_params(p.params)
    ...
    (; ᶜts, ᶜSqₗᵖ⁰, ᶜSqᵢᵖ⁰, ᶜSqᵣᵖ⁰, ᶜSqₛᵖ⁰, ᶜSnₗᵖ⁰, ᶜSnᵣᵖ⁰) = p.precomputed
    ᶜSᵖ = p.scratch.ᶜtemp_scalar
    ᶜSᵖ_2 = p.scratch.ᶜtemp_scalar_2

    # TODO - check if this is the correct velocity
    if p.atmos.turbconv_model isa DiagnosticEDMFX
        (; ᶜu⁰) = p.precomputed
    elseif p.atmos.turbconv_model isa EDOnlyEDMFX
        ᶜu⁰ = ᶜu
    ...
    # Aerosol activation
    # TODO - make it a user specified free parameter
    seasalt_num_prev_level = aerosol_params.c₀_seasalt
    seasalt_mean_radius_prev_level = aerosol_params.α_seasalt
    sulfate_num_prev_level = FT(0)
    ...
    @. ᶜSnₗᵖ⁰ += aerosol_activation_sources(
        ...
        specific(Y.c.ρn_liq, Y.c.ρ) + specific(Y.c.ρn_rai, Y.c.ρ),
        Y.c.ρ,
        max(0, w_component.(Geometry.WVector.(ᶜu⁰))),
        (microphysics_2m_params,),
        thermo_params,
        ᶜts,
        dt,
