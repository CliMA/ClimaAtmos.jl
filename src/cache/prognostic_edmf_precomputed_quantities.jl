...
NVTX.@annotate function set_prognostic_edmf_precomputed_quantities_environment!(
    ...
    (; ᶜtke⁰, ᶜρa⁰, ᶠu₃⁰, ᶜu⁰, ᶠu³⁰, ᶜK⁰, ᶜts⁰, ᶜρ⁰, ᶜmse⁰, ᶜq_tot⁰) =
        p.precomputed
    if p.atmos.moisture_model isa NonEquilMoistModel && p.atmos.precip_model isa Micophysics1M
    ...
    for j in 1:n
        ...
        if p.atmos.moisture_model isa NonEquilMoistModel && p.atmos.precip_model isa Micophysics1M
            @. ᶜtsʲ = TD.PhaseNonEquil_phq(
                ...
                ᶜp,
                ᶜmseʲ - ᶜΦ,
                TD.PhasePartition(ᶜq_totʲ, ᶜq_liqʲ + ᶜq_raiʲ, ᶜq_iceʲ + ᶜq_snoʲ)
            )
        else
...
NVTX.@annotate function set_prognostic_edmf_precomputed_quantities_bottom_bc!(
    ...
    for j in 1:n
        ...
        if p.atmos.moisture_model isa NonEquilMoistModel && p.atmos.precip_model isa Micophysics1M
            ...
            ρ_flux_q_rai_val = Fields.field_values(ρ_flux_q_rai)
            ρ_flux_q_sno_val = Fields.field_values(ρ_flux_q_sno)
        end
        ...
        if p.atmos.moisture_model isa NonEquilMoistModel && p.atmos.precip_model isa Micophysics1M
            ...
            @. ᶜq_liqʲ_int_val = sgs_scalar_first_interior_bc(
                ...
                ᶜq_liq_int_val,
                buoyancy_flux_val,
                ρ_flux_q_liq_val, #TODO - what is the correct value for it? (and below)
                ustar_val,
                obukhov_length_val,
                sfc_local_geometry_val,
            )
            ...
            ᶜq_rai_int_val = Fields.field_values(Fields.level(ᶜspecific.q_rai, 1))
            ᶜq_raiʲ_int_val = Fields.field_values(Fields.level(ᶜq_raiʲ, 1))
            @. ᶜq_raiʲ_int_val = sgs_scalar_first_interior_bc(
                ᶜz_int_val - z_sfc_val,
                ᶜρ_int_val,
                ᶜaʲ_int_val,
                ᶜq_rai_int_val,
            ...
            ᶜq_sno_int_val = Fields.field_values(Fields.level(ᶜspecific.q_sno, 1))
            ᶜq_snoʲ_int_val = Fields.field_values(Fields.level(ᶜq_snoʲ, 1))
            @. ᶜq_snoʲ_int_val = sgs_scalar_first_interior_bc(
                ᶜz_int_val - z_sfc_val,
                ᶜρ_int_val,
                ᶜaʲ_int_val,
                ᶜq_sno_int_val,
        ...
        if p.atmos.moisture_model isa NonEquilMoistModel && p.atmos.precip_model isa Micophysics1M
            @. ᶜtsʲ_int_val = TD.PhaseNonEquil_phq(
                ...
                TD.PhasePartition(ᶜq_totʲ_int_val, ᶜq_liqʲ_int_val + ᶜq_raiʲ_int_val, ᶜq_iceʲ_int_val + ᶜq_snoʲ_int_val),
            )
        else
...
NVTX.@annotate function set_prognostic_edmf_precomputed_quantities_precipitation!(
    ...
    # TODO - Tracer advection in the updrafts + tracer sedimentation
