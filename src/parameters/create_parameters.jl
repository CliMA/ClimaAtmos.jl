...
function ClimaAtmosParameters(
    ...
    # Unified 2M scheme always carries P3 ice (with_ice = true). The P3 size-
    # distribution quadrature order is exposed as a kwarg (default 40) so the
    # canonical runscript can sweep the speed/accuracy envelope.
    microphysics_2m_params = CM.Parameters.Microphysics2MParams(
        toml_dict;
        with_ice = true,
        quadrature_order = p3_quadrature_order,
    )
