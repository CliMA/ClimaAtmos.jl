...
"""
    AcousticSubstepTendency(G, G_lim, vertical, ν_d)
...
"""
struct AcousticSubstepTendency{G, GL, V, FT}
    G::G
    G_lim::GL
    vertical::V
    ν_d::FT
...
function (f::AcousticSubstepTendency)(Yₜ, Yₜ_lim, Y, p, t)
    ...
    if f.ν_d > 0
        @. Yₜ.c.uₕ += f.ν_d * wgradₕ(divₕ(Y.c.uₕ))
    end
    Yₜ .+= f.G
    Yₜ_lim .+= f.G_lim
    return nothing
...
"""
    AcousticMultirate(inner_alg, n_sub, vertical, outer_stages = 2, β_d = 0.0, implicit_split = false)
...
# Fields
  ...
  - `β_d`: divergence-damping coefficient.
...
"""
struct AcousticMultirate{A, V} <: CTS.TimeSteppingAlgorithm
    inner_alg::A
    n_sub::Int
    vertical::V
    outer_stages::Int
...
AcousticMultirate(inner_alg, n_sub, vertical) =
    AcousticMultirate(inner_alg, n_sub, vertical, 2, 0.0, false)
AcousticMultirate(inner_alg, n_sub, vertical, outer_stages) =
    AcousticMultirate(inner_alg, n_sub, vertical, outer_stages, 0.0, false)
AcousticMultirate(inner_alg, n_sub, vertical, outer_stages, β_d) =
    AcousticMultirate(inner_alg, n_sub, vertical, outer_stages, β_d, false)
...
function CTS.init_cache(prob, alg::AcousticMultirate; dt, kwargs...)
    ...
    forcing = AcousticSubstepTendency(G, G_lim, alg.vertical, ν_d)
...
# G = T_exp(u) - horizontal acoustic(u), evaluated and left in `G`/`G_lim`.
# Refreshes the cache `p` to be consistent with `u`.
...
function CTS.step_u!(integrator, cache::AcousticMultirateCache)
    (; f, G, G_lim, G2, G2_lim, A_buf, U0, n_sub, inner_integ, outer_integ, alg) =
        cache
    u = integrator.u
    p = integrator.p
    t = integrator.t
