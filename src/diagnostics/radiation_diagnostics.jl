...
function compute_rsd!(
    ...
    if isnothing(out)
        return copy(
            Fields.array2field(
                cache.radiation.rrtmgp_model.views.face_sw_flux_dn,
                axes(state.f),
            ),
        )
    else
...
function compute_rsu!(
    ...
    if isnothing(out)
        return copy(
            Fields.array2field(
                cache.radiation.rrtmgp_model.views.face_sw_flux_up,
                axes(state.f),
            ),
        )
    else
...
function compute_rld!(
    ...
    if isnothing(out)
        return copy(
            Fields.array2field(
                cache.radiation.rrtmgp_model.views.face_lw_flux_dn,
                axes(state.f),
            ),
        )
    else
...
function compute_rlu!(
    ...
    if isnothing(out)
        return copy(
            Fields.array2field(
                cache.radiation.rrtmgp_model.views.face_lw_flux_up,
                axes(state.f),
            ),
        )
    else
...
function compute_rsdcs!(
    ...
    if isnothing(out)
        return Fields.array2field(
            cache.radiation.rrtmgp_model.views.face_clear_sw_flux_dn,
            axes(state.f),
        )
    else
        out .= Fields.array2field(
...
function compute_rsucs!(
    ...
    if isnothing(out)
        return copy(
            Fields.array2field(
                cache.radiation.rrtmgp_model.views.face_clear_sw_flux_up,
                axes(state.f),
            ),
        )
    else
...
function compute_rldcs!(
    ...
    if isnothing(out)
        return copy(
            Fields.array2field(
                cache.radiation.rrtmgp_model.views.face_clear_lw_flux_dn,
                axes(state.f),
            ),
        )
    else
...
function compute_rlucs!(
    ...
    if isnothing(out)
        return copy(
            Fields.array2field(
                cache.radiation.rrtmgp_model.views.face_clear_lw_flux_up,
                axes(state.f),
            ),
        )
    else
...
function compute_reffclw!(
    ...
    if isnothing(out)
        return Fields.array2field(
            cache.radiation.rrtmgp_model.views.center_cloud_liquid_effective_radius,
            axes(state.c),
...
function compute_reffcli!(
    ...
    if isnothing(out)
        return Fields.array2field(
            cache.radiation.rrtmgp_model.views.center_cloud_ice_effective_radius,
            axes(state.c),
        ) .* FT(1e-6) # RRTMGP stores r_eff in microns
    else
        out .=
...
function compute_od550aer!(
    ...
    if isnothing(out)
        return Fields.array2field(
            cache.radiation.rrtmgp_model.views.aod_sw_extinction,
...
function compute_odsc550aer!(
    ...
    if isnothing(out)
        return Fields.array2field(
            cache.radiation.rrtmgp_model.views.aod_sw_scattering,
            axes(Fields.level(state.f, half)),
        )
    else
        out .= Fields.array2field(
