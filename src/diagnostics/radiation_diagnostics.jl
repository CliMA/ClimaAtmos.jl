...
function ᶠradiation_field_3d(state, cache, radiation_mode, getter::Function)
    ...
    # Flux getters are domain-masked views into the solver's plain (nlev, ncol)
    # presentation buffers, so `array2field` wraps them directly. Do NOT `copy`
    # (materializes a full field per call -- a large per-step allocation when many
    # radiation diagnostics run) or `parent` (un-masks the boundary level).
    flux = Fields.array2field(getter(cache.radiation.rrtmgp_solver), axes(state.f))
...
function ᶠradiation_field_sfc(state, cache, getter::Function)
    # Domain-masked view into the solver's plain presentation buffer; `array2field`
    # wraps it directly and `Fields.level` takes one level as a view -- no `copy`
    # (a full-field allocation per call) and no `parent` (un-masks the boundary level).
    Fields.level(
        Fields.array2field(getter(cache.radiation.rrtmgp_solver), axes(state.f)),
        half,
    )
end
...
compute_rsdscs(state, cache, _, ::RRTMGPI.AllSkyRadiationWithClearSkyDiagnostics) =
...
compute_rsuscs(state, cache, _, ::RRTMGPI.AllSkyRadiationWithClearSkyDiagnostics) =
...
compute_rldscs(state, cache, _, ::RRTMGPI.AllSkyRadiationWithClearSkyDiagnostics) =
