...
compute_arup!(_, _, _, _, turbconv) = error_diagnostic_variable("arup", turbconv)

function compute_arup!(out, state, cache, time, turbconv::PrognosticEDMFX)
    ρaʲ = (state.c.sgsʲs.:1).ρa
    ρʲ = compute_rhoaup!(out, state, cache, time, turbconv)
    return draft_area(ρaʲ, ρʲ)
end
function compute_arup!(out, state, cache, time, turbconv::DiagnosticEDMFX)
...
compute_rhoaup!(_, _, _, _, turbconv) = error_diagnostic_variable("rhoaup", turbconv)
...
compute_waup!(_, _, _, _, turbconv) = error_diagnostic_variable("waup", turbconv)
...
compute_taup!(_, _, _, _, turbconv) = error_diagnostic_variable("taup", turbconv)
...
compute_thetaaup!(_, _, _, _, turbconv) = error_diagnostic_variable("thetaaup", turbconv)
...
compute_haup!(_, _, _, _, turbconv) = error_diagnostic_variable("haup", turbconv)
...
compute_entr!(_, _, _, _, turbconv) = error_diagnostic_variable("entr", turbconv)
...
compute_turbentr!(_, _, _, _, turbconv) = error_diagnostic_variable("turbentr", turbconv)
...
compute_detr!(_, _, _, _, turbconv) = error_diagnostic_variable("detr", turbconv)
...
compute_aren!(_, _, _, _, turbconv) = error_diagnostic_variable("aren", turbconv)
...
function compute_aren!(out, state, cache, time, turbconv_model::PrognosticEDMFX)
    ...
    ᶜρ⁰ = compute_rhoaen!(out, state, cache, time, turbconv_model)
    return @. lazy(draft_area(ᶜρa⁰, ᶜρ⁰))
end

function compute_aren!(out, state, cache, time, turbconv::DiagnosticEDMFX)
    ᶜρaʲ = cache.precomputed.ᶜρaʲs.:1
    ᶜρʲ = compute_rhoaup!(out, state, cache, time, turbconv)
...
compute_rhoaen!(_, _, _, _, turbconv) = error_diagnostic_variable("rhoaen", turbconv)
...
compute_waen!(_, _, _, _, turbconv) = error_diagnostic_variable("waen", turbconv)
...
compute_taen!(_, _, _, _, turbconv) = error_diagnostic_variable("taen", turbconv)
...
compute_thetaaen!(_, _, _, _, turbconv) = error_diagnostic_variable("thetaaen", turbconv)
...
compute_haen!(_, _, _, _, turbconv) = error_diagnostic_variable("haen", turbconv)
...
compute_tke!(_, _, _, _, turbconv) = error_diagnostic_variable("tke", turbconv)

function compute_tke!(_, state, _, _,
    ::Union{EDOnlyEDMFX, PrognosticEDMFX, DiagnosticEDMFX},
)
    ᶜρa⁰_pedmfx = @. lazy(ρa⁰(state.c.ρ, state.c.sgsʲs, turbconv_model))
    ᶜρa⁰ = turbconv_model isa PrognosticEDMFX ? ᶜρa⁰_pedmfx : state.c.ρ
...
compute_lmixw!(_, _, _, _, turbconv) = error_diagnostic_variable("lmixw", turbconv)
...
compute_lmixtke!(_, _, _, _, turbconv) = error_diagnostic_variable("lmixtke", turbconv)
...
compute_lmixb!(_, _, _, _, turbconv) = error_diagnostic_variable("lmixb", turbconv)
...
compute_edt!(_, _, _, _, vertical_diffusion, turbconv) =
    error_diagnostic_variable(
...
function compute_edt!(out, state, cache, time,
    vertdiff::Nothing, turbconv::Union{PrognosticEDMFX, DiagnosticEDMFX},
)
    (; ᶜlinear_buoygrad, ᶜstrain_rate_norm) = cache.precomputed
    ᶜK_u = compute_evu!(out, state, cache, time, vertdiff, turbconv)
    ᶜprandtl_nvec =
...
function compute_evu!(out, state, cache, time,
    ::Nothing, turbconv::Union{PrognosticEDMFX, DiagnosticEDMFX},
)
    turbconv_params = CAP.turbconv_params(cache.params)
    ᶜtke⁰ = compute_tke!(out, state, cache, time, turbconv)
