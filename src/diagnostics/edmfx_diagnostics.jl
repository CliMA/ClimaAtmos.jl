...
###
# Tendency of temperature due to SGS mass flux (3d)
###
compute_tntmf(state, cache, time) = compute_tntmf(state, cache, time,
    cache.atmos.microphysics_model, cache.atmos.turbconv_model,
)
compute_tntmf(_, _, _, _, _) =
    error_diagnostic_variable("Can only compute tendency of temperature due to SGS mass flux \
                               with a moist model and with EDMFX")

function compute_tntmf(state, cache, _,
    ::MoistMicrophysics, turbconv_model::PrognosticEDMFX,
)
    (; edmfx_sgsflux_upwinding) = cache.atmos.numerics
    (; ᶠu³) = cache.precomputed
    ...
    ᶜtnenemf = @. lazy(vtt_eʲ + vtt_e⁰)

    # --- Humidity mass flux tendency ---
    ᶜq_tot = @. lazy(specific(state.c.ρq_tot, state.c.ρ))
    ᶜq_totʲ = (state.c.sgsʲs.:1).q_tot
    ᶜa_scalar_qʲ = @. lazy(
        (ᶜq_totʲ - ᶜq_tot) * draft_area(ᶜρaʲ, ᶜρʲ),
    )
    vtt_qʲ = vertical_transport(
        ᶜρʲ,
        ᶠu³_diffʲ,
        ᶜa_scalar_qʲ,
        dt,
    ...
    ᶜtnhusmf = @. lazy(vtt_qʲ + vtt_q⁰)

    # --- Convert energy tendency to temperature tendency ---
    # From: ρ * de/dt = ρ * cv_m * dT/dt + ρ * (cv_v*(T-T_0) + Lv_0 - R_v*T_0) * dqt/dt
    # So:   dT/dt = (de/dt - (cv_v*(T-T_0) + Lv_0 - R_v*T_0) * dqt/dt) / cv_m
    ...
    return @. lazy(
        (
            ᶜtnenemf / state.c.ρ -
            (cv_v * (ᶜT - T_0) + Lv_0 - R_v * T_0) *
            ᶜtnhusmf / state.c.ρ
        ) / TD.cv_m(thermo_params, ᶜq_tot_nonneg, ᶜq_liq, ᶜq_ice),
    )
end

add_diagnostic_variable!(short_name = "tntmf", units = "K s^-1",
    long_name = "Tendency of temperature due to SGS mass flux transport",
    comments = "Temperature tendency computed from the SGS mass flux energy and humidity tendencies, including updraft and environment contributions",
    compute = compute_tntmf,
)
