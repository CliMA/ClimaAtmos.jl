...
function compute_arup!(_, state, cache, _, turbconv::PrognosticEDMFX)
    ρaʲ = (state.c.sgsʲs.:1).ρa
    ρʲ = compute_rhoaup!(-, -, cache, -, turbconv)
    return @. lazy(draft_area(ρaʲ, ρʲ))
end
function compute_arup!(_, _, cache, _, turbconv::DiagnosticEDMFX)
...
function compute_edt!(_, state, cache, _,
    ...
    ᶜK_u = compute_evu!(-, state, cache, -, vertdiff, turbconv)
    ᶜprandtl_nvec =
...
function compute_evu!(_, state, cache, _,
    ...
    ᶜtke⁰ = compute_tke!(-, state, -, -, turbconv)
