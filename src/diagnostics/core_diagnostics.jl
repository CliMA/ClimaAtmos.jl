...
#
# In addition to the metadata (names, comments, ...), the most
# important step in adding a new DiagnosticVariable is defining
# its compute function. `compute` takes three arguments:
# (state, cache, time), and returns the diagnostic value,
# preferring lazy fields when possible.
#
# Often, it is possible to compute certain diagnostics only for
# specific models (e.g., humidity for moist models). For that,
# it is convenient to adopt the following pattern:
#
# 1. Define a function for the case we know how to handle:
#
# compute_hur(state, cache, time, ::MoistMicrophysics) =
#     @. lazy(TD.relative_humidity(thermo_params, cache.precomputed.ᶜT, ...))
#
...
#
# compute_hur(state, cache, time, model) =
...
# Density (3d)
###
compute_rhoa(state, _, _) = state.c.ρ
...
# U velocity (3d)
###
compute_ua(_, cache, _) = @. lazy(u_component(Geometry.UVector(cache.precomputed.ᶜu)))
...
# V velocity (3d)
###
compute_va(_, cache, _) = @. lazy(v_component(Geometry.VVector(cache.precomputed.ᶜu)))
...
# W velocity (3d)
###
# TODO: may want to convert to omega (Lagrangian pressure
# tendency) as standard output
compute_wa(_, cache, _) = @. lazy(w_component(WVec(cache.precomputed.ᶠu)))
...
# Temperature (3d)
###
compute_ta(_, cache, _) = cache.precomputed.ᶜT
...
function compute_thetaa(state, cache, _)
    ...
    return @. lazy(
        TD.potential_temperature(
            thermo_params, ᶜT, state.c.ρ, ᶜq_tot_safe, ᶜq_liq_rai, ᶜq_ice_sno,
        ),
    )
end
...
# Air pressure (3d)
###
compute_pfull(_, cache, _) = cache.precomputed.ᶜp
...
function compute_rv(_, cache, _)
    vort = @. w_component(WVec(wcurlₕ(cache.precomputed.ᶜu)))
...
###
compute_zg(_, cache, _) = @. lazy(cache.core.ᶜΦ / CAP.grav(cache.params))
...
# Cloud fraction (3d)
###
compute_cl(_, cache, _) = @. lazy(cache.precomputed.ᶜcloud_fraction * 100)
...
###
compute_ke(_, cache, _) = cache.precomputed.ᶜK
...
###
compute_bgrad(_, cache, _) = cache.precomputed.ᶜlinear_buoygrad
...
###
compute_strain(_, cache, _) = cache.precomputed.ᶜstrain_rate_norm
...
compute_clw(_, cache, _, ::EquilibriumMicrophysics0M) = cache.precomputed.ᶜq_liq_rai

compute_clw(state, _, _, ::NonEquilibriumMicrophysics) =
    @. lazy(specific(state.c.ρq_liq, state.c.ρ))
...
compute_cli(_, cache, _, ::EquilibriumMicrophysics0M) = cache.precomputed.ᶜq_ice_sno
...
###
compute_ts(_, cache, _) = cache.precomputed.sfc_conditions.T_sfc
...
###
compute_tas(_, cache, _) = Fields.level(cache.precomputed.ᶜT, 1)
...
function compute_uas(_, cache, _)
    ...
    return @. lazy(u_component(Geometry.UVector(ᶜu₁)))
end
...
function compute_vas(_, cache, _)
    ᶜu₁ = Fields.level(cache.precomputed.ᶜu, 1)
    return @. lazy(v_component(Geometry.VVector(ᶜu₁)))
end
...
function compute_tau(cache, component)
    ...
    ρ_flux = @. lazy(Geometry.UVVector(adjoint(ρ_flux_uₕ) * surface_ct3_unit))
    return getproperty(ρ_flux.components.data, component)  # TODO: this allocates
end
...
compute_cdnc(state, _, _, ::NonEquilibriumMicrophysics2M) = state.c.ρn_liq
...
compute_orog(state, _, _, ::Grids.Flat) = @. lazy(zero(Fields.level(state.c, 1)))
...
function compute_clwvi(state, cache, _, ::EquilibriumMicrophysics0M)
    ...
    (; ᶜq_liq_rai, ᶜq_ice_sno) = cache.precomputed
    clw = @. lazy(state.c.ρ * (ᶜq_liq_rai + ᶜq_ice_sno))
...
function compute_clwvi(state, cache, _, ::NonEquilibriumMicrophysics)
    out = cache.scratch.ᶠtemp_field_level
    clw = @. lazy(state.c.ρq_liq + state.c.ρq_ice)
    Operators.column_integral_definite!(out, clw)
    return out
end
...
function compute_lwp(state, cache, _, ::EquilibriumMicrophysics0M)
    out = cache.scratch.ᶠtemp_field_level
    lw = @. lazy(state.c.ρ * cache.precomputed.ᶜq_liq_rai)
...
function compute_clivi(state, cache, _, ::MoistMicrophysics)
    out = cache.scratch.ᶠtemp_field_level
    cli = @. lazy(state.c.ρ * cache.precomputed.ᶜq_ice_sno)
...
function compute_hurvi(state, cache, _, ::MoistMicrophysics)
    ...
    sat = @. lazy(
        state.c.ρ * TD.q_vap_saturation(
            thermo_params, ᶜT, state.c.ρ, ᶜq_liq_rai, ᶜq_ice_sno,
        ),
    )
...
function compute_husv(_, cache, _, ::MoistMicrophysics)
    ...
    return @. lazy(ᶜq_tot_safe - ᶜq_liq_rai - ᶜq_ice_sno)
end
...
function compute_cape!(out, state, cache, _)
    (; ᶜT, ᶜq_tot_safe, ᶜq_liq_rai, ᶜq_ice_sno, ᶜp, sfc_conditions) = cache.precomputed
    thermo_params = CAP.thermodynamics_params(cache.params)
    ...
    surface_θ_liq_ice =
        lazy.(
            TD.liquid_ice_pottemp_given_pressure.(
                thermo_params, surface_T, surface_p, surface_q,
            ),
        )
    ...
    _parcel_T_from_sa(thermo_params, p, θ_liq_ice, q_tot, maxiter) =
        TD.saturation_adjustment(thermo_params, TD.pθ_li(), p, θ_liq_ice, q_tot; maxiter).T

    # Create parcel thermodynamic states at each level based on energy & moisture at surface
    parcel_T = lazy.(_parcel_T_from_sa.(thermo_params, ᶜp, surface_θ_liq_ice, surface_q, 4))
...
function compute_mslp(state, cache, _)
    (; ᶜq_tot_safe, ᶜq_liq_rai, ᶜq_ice_sno, ᶜp, ᶜT) = cache.precomputed
    thermo_params = CAP.thermodynamics_params(cache.params)
    g = TD.Parameters.grav(thermo_params)
    q_tot_safe_level = Fields.level(ᶜq_tot_safe, 1)
    q_liq_rai_level = Fields.level(ᶜq_liq_rai, 1)
    q_ice_sno_level = Fields.level(ᶜq_ice_sno, 1)
    R_m_surf = @. lazy(
        TD.gas_constant_air(
            thermo_params, q_tot_safe_level, q_liq_rai_level, q_ice_sno_level,
        ),
    )
