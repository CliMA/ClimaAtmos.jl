...
add_diagnostic_variable!(short_name = "ni", units = "kg⁻¹",
...
add_diagnostic_variable!(short_name = "qrim", units = "kg kg⁻¹",
    long_name = "Rime Mass Specific Content",
    comments = "Mass of rime on P3 ice per unit mass of air.",
    compute = compute_qrim,
)

import CloudMicrophysics.Utilities: rime_mass_fraction, rime_density
...
compute_frim(state, _, _, ::NonEquilibriumMicrophysics2M) =
    @. lazy(rime_mass_fraction(state.c.ρq_rim, state.c.ρq_ice))
...
compute_rhorim(state, _, _, ::NonEquilibriumMicrophysics2M) =
    @. lazy(rime_density(state.c.ρq_rim, state.c.ρb_rim))
