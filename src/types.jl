...
"""
    AtmosAerosols(prescribed_names, prognostic_names; prognostic_aerosol_params)

Resolve tuples of species keys into NamedTuple of `AbstractAerosolModels`
via [`AEROSOL_SPECIES`](@ref); `prognostic_names` takes precedence when a 
species is double listed.
"""
function AtmosAerosols(
    ...
    models = map(AEROSOL_SPECIES) do (species, prescribed, prognostic)
...
const AEROSOL_SPECIES = (;
    seasalt = (
        "SSLT",
        PrescribedSeaSalt,
        ap -> PrognosticSeaSalt(length(ap.ssa_bin_edges) - 1),
    ),
    dust = ("DST", PrescribedDust, nothing),
    sulfate = ("SO4", PrescribedSulfate, nothing),
    black_carbon = ("CB", PrescribedBlackCarbon, nothing),
    organic_carbon = ("OC", PrescribedOrganicCarbon, nothing),
)
