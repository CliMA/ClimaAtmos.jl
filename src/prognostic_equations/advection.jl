...
NVTX.@annotate function horizontal_dynamics_tendency!(Yₜ, Y, p, t)
    ...
    # local_grad = lazy.(Operators.GradientF2C(
    #        bottom = Operators.Extrapolate(),
    #        top = Operators.Extrapolate(),
    #    ))
    # ᶜdTdz = p.scratch.ᶜtemp_scalar
    # @. ᶜdTdz = Geometry.WVector.(local_grad.(ᶠinterp.(T_ref))).components.data.:1
    # ᶜdTdz0 = p.scratch.temp_field_level
    # @. ᶜdTdz0 = Fields.level(ᶜdTdz, 1)

    # T_ref_surf = thermo_params.T_surf_ref
    # T0 = p.scratch.temp_field_level
    # @. T0 = T_ref_surf + ᶜdTdz0*T_ref_surf*cp_d / CAP.grav(params)
    T0 = T_ref
