...
- `ρ_flux_uₕ_surface`: The vertical flux of horizontal momentum through the bottom
  ...
  `C3{Nothing} ⊗ C12{FT}` (e.g., representing surface stress `τ` as
  `e_3 ⊗ τ` if defined as flux into the domain, or simply
  the stress vector `τ` if the `SetValue` operator handles the normal).
   Conventionally, a positive flux might represent momentum transfer from the
  atmosphere to the surface (drag).
...
"""
    boundary_tendency_scalar(ᶜχ_field_template, ρ_flux_χ_surface)
...
Arguments:
- `ᶜχ_field_template`: A cell-center scalar field of the same type as the quantity
  `χ` for which the tendency is being computed (used for eltype and spatial structure,
