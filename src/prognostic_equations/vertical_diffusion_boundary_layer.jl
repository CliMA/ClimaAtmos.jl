...
function vertical_diffusion_boundary_layer_tendency!(
    ...
    FT = eltype(Y)
    (; ᶜu, ᶜspecific, ᶜK_u, ᶜK_h, sfc_conditions) = p.precomputed
    ᶠgradᵥ = Operators.GradientC2F() # apply BCs to ᶜdivᵥ, which wraps ᶠgradᵥ
...
function vertical_diffusion_boundary_layer_tendency_uₕ(
    ᶜρ,
    ᶜu,
    ᶜK_u,
    ρ_flux_uₕ,
    vert_diff::Union{VerticalDiffusion, FriersonDiffusion, DecayWithHeightDiffusion},
)
    if diffuse_momentum(vert_diff)
        FT = Spaces.undertype(axes(ᶜρ))
        ᶠgradᵥ = Operators.GradientC2F() # apply BCs to ᶜdivᵥ, which wraps ᶠgradᵥ
        ᶜu = 
        bc_strain_rate = compute_strain_rate_face(ᶜu)
        bc1 = @lazy @ - C12(
            ᶜdivᵥ(-2 * ᶠinterp(ᶜρ) * ᶠinterp(ᶜK_u) * bc_strain_rate) / ᶜρ,
        )
        ...
        bc2 = @lazy @. - ᶜdivᵥ_uₕ(-(FT(0) * ᶠgradᵥ(uₕ))) / ᶜρ
        bc = @lazy @. bc1 + bc2
        return bc
    end
