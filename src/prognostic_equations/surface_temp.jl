...
function surface_temp_tendency!(Yₜ, Y, p, t, slab::EisenmanSeaIce)
    ...
    if (h_ice > 0.0) # ice-covered
        # hold tendencies at zero for now
        @. Yₜ.sfc.h_ice = FT(0)
        @. Yₜ.sfc.T_ml = FT(0)
        @. Yₜ.sfc.T_ice = FT(0)
        @. Yₜ.sfc.water = FT(0)
    elseif (h_ice ≤ 0.0) # ice free
        # --- ENERGY BALANCE ---
        # Denominator for temperature tendency
        surface_heat_capacity_per_area = ρ_ocean * cp_ocean * depth_ocean
        ...
        error("Invalid sea-ice thickness value: h_ice = $h_ice")
    end
end
