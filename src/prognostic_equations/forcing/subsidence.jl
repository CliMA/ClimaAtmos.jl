...
The tendency is calculated as `ρ * (-w * ∂χ/∂z)`, where `w` is `ᶠu³` and `χ` is `ᶜχ`.
...
upwind scheme) to compute and apply the advective tendency `ρ * (-w_sub * ∂χ/∂z)`
for each relevant scalar quantity `χ`.
