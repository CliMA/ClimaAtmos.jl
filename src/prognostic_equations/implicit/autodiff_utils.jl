...
# An iterator with pairs of the form (scalar_level_index, (name_index, level)),
# where name_index is an index into scalar_field_names(field_vector), level is
# a vertical index into the scalar field with this name, and scalar_level_index
# is a unique linear index for each combination of scalar field name and level.
function scalar_level_pair_iterator(field_vector)
    scalar_names = scalar_field_names(field_vector)
    name_index_and_level_pairs =
        unrolled_map(enumerate(scalar_names)) do (name_index, name)
            field = MatrixFields.get_field(field_vector, name)
            is_one_level = field isa Fields.SpectralElementField
            Iterators.map(
                level -> (name_index, level),
                Base.OneTo(is_one_level ? 1 : Spaces.nlevels(axes(field))),
            )
        end
    return enumerate(Iterators.flatten(name_index_and_level_pairs))
end

# A view of the data for one point in a field, which can be used like a Ref.
Base.@propagate_inbounds function point(field, level, column_index...)
    column_field = Fields.column(field, column_index...)
    column_field isa Fields.PointField && return column_field
    first_level_index = Operators.left_idx(axes(column_field))
    return Fields.level(column_field, first_level_index + level - 1)
end
...
# for `Dual` in `ForwardDiff` and breaks precompilation, while specializing on
# the default tag `Nothing` causes the type piracy test to fail.
...
# the derivative of `x * sqrt(x)` is `NaN` when `x` is zero.
