...
function update_jacobian!(alg::ExactJacobian, cache, Y, p, dtγ, t)
    ...
    Yₜ_dual_columns = column_iterator(Yₜ_dual)
    ...
    for level_batch in level_batches
        ...
        for (partial_index, (ε_index, _)) in enumerate(level_batch)
            for (col_index, Yₜ_dual_column) in enumerate(Yₜ_dual_columns)
                column_matrix_slice =
                    view(column_matrices, col_index, :, ε_index)
                vector_to_fieldvector(column_matrix_slice, Yₜ_dual_column) .=
                    getindex.(
                        ForwardDiff.partials.(Yₜ_dual_column),
                        partial_index,
                    )
...
function save_jacobian!(alg::ExactJacobian, cache, Y, dtγ, t)
    (; column_matrices, column_vectors, column_matrix, column_vector) = cache
    (n_columns, n_εs) = size(column_vectors)

    column_matrix .= view(column_matrices, 1, :, :)
    column_vector .= view(column_vectors, 1, :)
    file_name = "exact_jacobian$(n_columns == 1 ? "" : "_first")"
