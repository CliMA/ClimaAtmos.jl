@kwdef struct ExactJacobian <: JacobianAlgorithm
    differentiate_all_precomputed::Bool = true
...
#=
struct DenseFieldMatrix{preserve_unfactorized_matrices, M, F, R}
    column_matrices::M
    factorized_column_matrices::F
    index_range_map::R
end
DenseFieldMatrix{preserve_unfactorized_matrices}(
    column_matrices::M,
    factorized_column_matrices::F,
    index_range_map::R,
) where {M, F, R} = DenseFieldMatrix{preserve_unfactorized_matrices, M, F, R}(
    column_matrices,
    factorized_column_matrices,
    index_range_map,
)
function DenseMatrixField(field_vector; preserve_unfactorized_matrices = false)
    FT = eltype(field_vector)
    DA = ClimaComms.array_type(field_vector)
    columns = column_iterator(field_vector)
    n_columns = length(columns)
    matrix_size = length(first(columns))
    column_matrices = DA{FT}(undef, n_columns, matrix_size, matrix_size)
    factorized_column_matrices =
        preserve_unfactorized_matrices ? copy(column_matrices) : column_matrices
    names = scalar_field_names(field_vector)
    index_ranges = scalar_field_index_ranges(field_vector)
    name_index_range_pairs =
        MatrixFields.unrolled_map(Pair, names, index_ranges)
    index_range_map = MatrixFields.FieldNameDict{Any}(name_index_range_pairs...)
    return DenseFieldMatrix{preserve_unfactorized_matrices}(
        column_matrices,
        factorized_column_matrices,
        index_range_map,
    )
