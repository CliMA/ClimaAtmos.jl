...
function update_jacobian!(alg::ManualSparseJacobian, cache, Y, p, dtγ, t)
    ...
    if use_derivative(diffusion_flag)
        ...
        foreach_gs_tracer(Yₜ, Y) do ᶜρχₜ, ᶜρχ, ρχ_name
            ...
            ᶜtridiagonal_matrix_scalar = if ρχ_name in (@name(c.ρq_rai), @name(c.ρq_sno), @name(c.ρn_rai))
                ᶜdiffusion_h_matrix_scaled
            else
                ᶜdiffusion_h_matrix
            end
        ...
        if MatrixFields.has_field(Y, @name(c.sgs⁰.ρatke))
            ...
            ᶜρa⁰ = p.atmos.turbconv_model isa PrognosticEDMFX ? @.lazy(ρa⁰(Y.c)) : ᶜρ
