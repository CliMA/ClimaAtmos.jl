...
function jacobian_cache(alg::AutoSparseJacobian, Y, atmos)
    ...
    for block_row_name in scalar_names, block_column_name in scalar_names
        ...
        if (block_row_name, block_column_name) in keys(autodiff_matrix)
            ...
            (lower_band, upper_band) =
                n_band_matrix_rows == n_band_matrix_columns ? (1 / 2, -1 / 2) :
    ...
    # deteriorate if we only store a single column of Y_dual_εs.
    Y_dual = replace_parent_type(Y, FT_dual)
    Y_dual_εs = similar(Y_dual)
    ...
    # as a sparse N × n_εs representation of ∂Y/∂Y. n_εs is wrapped in a Val and
    # Y_index_to_ε_index_map is converted to a DA for GPU compatibility.
    n_εs_val = Val(n_εs)
    ClimaComms.@threaded device begin
        # On multithreaded devices, assign one thread to each dual number.
        for column_index in column_indices,
            index_pair in DA(collect(Y_index_to_ε_index_map))

            ((scalar_index, level_index), ε_index) = index_pair
            Y_dual_εs_value =
    ...
    band_matrix_row_index_to_ε_indices_maps =
        map(enumerate(pairs(autodiff_matrix))) do block_index_and_pair
            (block_index, (block_key, matrix_field)) = block_index_and_pair
...
function update_jacobian!(::AutoSparseJacobian, cache, Y, p, dtγ, t)
    ...
    # Move the entries of ∂Yₜ/∂Y from Yₜ_dual into autodiff_matrix. Drop spatial
    # information from all fields to stay below the GPU parameter memory limit.
    Yₜ_dual_without_spaces =
        unrolled_map(Fields.field_values, Fields._values(Yₜ_dual))
    matrix_fields_without_spaces =
    ...
    ClimaComms.@threaded device begin
        ...
        for column_index in column_indices,
            ...
            Yₜ_dual_value = unrolled_applyat(scalar_index, scalar_names) do name
                data = MatrixFields.get_field(Yₜ_dual_without_spaces, name)
                @inbounds point(data, level_index, column_index...)[]
            end
            Yₜ_ε_values = ForwardDiff.partials(Yₜ_dual_value)
            unrolled_applyat(
                block_index,
                matrix_fields_without_spaces,
            ) do matrix_data
                (lower_band, upper_band) =
                    MatrixFields.outer_diagonals(eltype(matrix_data))
                band_matrix_row_entries =
                    ntuple(Val(upper_band - lower_band + 1)) do band_index
                        FT = eltype(Yₜ_ε_values)
                        ε_index = ε_indices[band_index]
                        ε_index > 0 ? Yₜ_ε_values[ε_index] : FT(0)
                    end
                @inbounds point(matrix_data, level_index, column_index...)[] =
                    eltype(matrix_data)(band_matrix_row_entries)
            end
        end
    end
