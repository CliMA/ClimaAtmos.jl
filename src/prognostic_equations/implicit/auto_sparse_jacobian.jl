...
function jacobian_cache(alg::AutoSparseJacobian, Y, atmos)
    ...
    field_vector_indices = collect(field_vector_index_iterator(Y)) # iterator of (f, v)

    # TODO: Add FieldNameTree(Y) to the matrix in FieldMatrixWithSolver.
    ...
    # TODO: Keep the FieldNameTree of tendency_matrix in scalar_fieldmatrix.
    # scalar_tendency_matrix = MatrixFields.scalar_fieldmatrix(tendency_matrix)
    scalar_tendency_matrix = MatrixFields.replace_name_tree(
        MatrixFields.scalar_fieldmatrix(tendency_matrix),
        tree,
    )
    ...
    band_matrix_row_index_to_ε_indices_maps =
        map(enumerate(pairs(autodiff_matrix))) do block_index_and_pair
            ...
            block_Yₜ_indices = filter(field_vector_indices) do (scalar_index, _)
                scalar_names[scalar_index] == scalar_block_row_name
            end
