...
inferred_derivative_flag(maybe_flag, default_value) =
...
nested linear solver. The approximation also has flags for setting certain
derivatives to 0; these are represented as `DerivativeFlag`s (either
`UseDerivative` or `Ignoreerivative`) instead of `Bool`s for type-stability.
...
- `topography_flag::DerivativeFlag`: whether the derivative of vertical
  ...
  computed or approximated as 0
- `diffusion_flag::DerivativeFlag`: whether the derivatives of the grid-scale
  diffusion tendency should be computed or approximated as 0
- `sgs_advection_flag::DerivativeFlag`: whether the derivatives of the
  subgrid-scale advection tendency should be computed or approximated as 0
- `sgs_mass_flux_flag::DerivativeFlag`: whether the derivatives of the
  subgrid-scale mass flux tendency should be computed or approximated as 0
...
"""
@kwdef struct ApproxJacobian{F1, F2, F3, F4} <: JacobianAlgorithm
    topography_flag::F1 = nothing
    diffusion_flag::F2 = nothing
    sgs_advection_flag::F3 = nothing
    sgs_mass_flux_flag::F4 = nothing
    approximate_solve_iters::Int = 1
end

function jacobian_cache(alg::ApproxJacobian, Y, atmos)
    topography_flag =
        inferred_derivative_flag(alg.topography_flag, has_topography(axes(Y.c)))
    diffusion_flag =
        inferred_derivative_flag(alg.diffusion_flag, atmos.diff_mode)
    sgs_advection_flag =
        inferred_derivative_flag(alg.sgs_advection_flag, atmos.sgs_adv_mode)
    sgs_mass_flux_flag =
        inferred_derivative_flag(alg.sgs_mass_flux_flag, atmos.sgs_mf_mode)
...
function approximate_jacobian!(::ApproxJacobian, cache, Y, p, dtγ, t)
    ...
    (; ᶜΦ, ᶠgradᵥ_ᶜΦ) = p.core
    (; ᶜspecific, ᶜK, ᶜts, ᶜp, ᶜh_tot) = p.precomputed
    (;
        ∂ᶜK_∂ᶜuₕ,
        ∂ᶜK_∂ᶠu₃,
        ᶠp_grad_matrix,
    ...
    if p.atmos.turbconv_model isa PrognosticEDMFX
        if use_derivative(sgs_advection_flag)
            (; ᶜgradᵥ_ᶠΦ) = p.core
            (; ᶜρʲs, ᶠu³ʲs, ᶜtsʲs) = p.precomputed
            (; bdmr_l, bdmr_r, bdmr) = p.precomputed
            ...
            if use_derivative(sgs_mass_flux_flag)
                (; ᶜgradᵥ_ᶠΦ) = p.core
                (; ᶜρʲs, ᶠu³ʲs, ᶜtsʲs, ᶠu³) = p.precomputed
                (; bdmr_l, bdmr_r, bdmr) = p.precomputed
                is_third_order = edmfx_upwinding == Val(:third_order)
                ᶠupwind = is_third_order ? ᶠupwind3 : ᶠupwind1
                ᶠset_upwind_bcs = Operators.SetBoundaryOperator(;
