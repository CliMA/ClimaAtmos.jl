...
function update_jacobian!(alg::ApproxJacobian, cache, Y, p, dtγ, t)
    ...
    if p.atmos.moisture_model isa NonEquilMoistModel &&
        use_derivative(noneq_cloud_formation_flag)
         ...
         function ∂ρqₗ_err_∂ρqᵪ(tps, ts, deriv)
             FT_inner = eltype(tps)
         ...
         function ∂ρqᵢ_err_∂ρqᵪ(tps, ts, deriv)
             FT_inner = eltype(tps)
             q = TD.PhasePartition(tps, ts)
             ρ = TD.air_density(tps, ts)
         ...
         @. ∂ᶜρqₗ_err_∂ᶜρqₗ +=
             DiagonalMatrixRow(
                 ∂ρqₗ_err_∂ρqᵪ(
                     thermo_params, ᶜts, (-1 / (τₗ * Γₗ(thermo_params, ᶜts))),
                 )
             )
         ...
         @. ∂ᶜρqᵢ_err_∂ᶜρqᵢ +=
             DiagonalMatrixRow(
                 ∂ρqᵢ_err_∂ρqᵪ(
                     thermo_params, ᶜts, (-1 / (τᵢ * Γᵢ(thermo_params, ᶜts))),
                     )
                 )
         ...
         @. ∂ᶜρqₗ_err_∂ᶜρqₜ = DiagonalMatrixRow(
             ∂ρqₗ_err_∂ρqᵪ(
                 thermo_params, ᶜts, ((1 - ᶜρ * ᶜ∂qₛₗ_∂p * ᶜ∂p_∂ρqₜ) / (τₗ * Γₗ(thermo_params, ᶜts))),
             )
         )
         ...
         @. ∂ᶜρqᵢ_err_∂ᶜρqₜ = DiagonalMatrixRow(
             ∂ρqᵢ_err_∂ρqᵪ(
                 thermo_params, ᶜts, ((1 - ᶜρ * ᶜ∂qₛᵢ_∂p * ᶜ∂p_∂ρqₜ) / (τᵢ * Γᵢ(thermo_params, ᶜts))),
             )
         )
     end
