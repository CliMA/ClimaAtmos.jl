...
NVTX.@annotate function save_jacobian!(jacobian, Y, p, dtγ, t)
    (; Yₜ, column_vectors) = jacobian.cache
    ...
    for (col_index, Yₜ_column) in enumerate(column_iterator(Yₜ))
        column_vector_view = view(column_vectors, col_index, :)
        vector_to_fieldvector(column_vector_view, Yₜ_column) .= Yₜ_column
