...
function update_column_matrices!(alg::AutoDenseJacobian, cache, Y, p, dtγ, t)
    ...
    scalar_level_pairs = scalar_level_pair_iterator(Y)
    ...
    batches = Iterators.partition(scalar_level_pairs, batch_size)
    for batch in ClimaComms.threadable(device, batches)
        # Add a unique ε to Y for each scalar level index in this batch.
        Y_dual .= Y
        ClimaComms.@threaded device begin
            for column_index in column_indices,
                (ε_index, (_, (name_index, level))) in enumerate(batch)

                partials = ntuple(i -> i == ε_index ? 1 : 0, batch_size_val)
                dual_value = ForwardDiff.Dual{Jacobian}(0, partials...)
                unrolled_applyat(name_index, scalar_names) do name
                    field = MatrixFields.get_field(Y_dual, name)
                    @inbounds point(field, level, column_index...)[] +=
                        dual_value
                end
            end
        end
        ...
        # Copy this batch's portion of ∂Yₜ/∂Y into column_matrices.
        ClimaComms.@threaded device begin
            for (matrix_index, column_index) in enumerate(column_indices),
                (ε_index, (scalar_level_index2, _)) in enumerate(batch),
                (scalar_level_index1, (name_index, level)) in scalar_level_pairs

                dual_value = unrolled_applyat(name_index, scalar_names) do name
                    field = MatrixFields.get_field(Yₜ_dual, name)
                    @inbounds point(field, level, column_index...)[]
                end
                @inbounds column_matrices[
                    scalar_level_index1,
                    scalar_level_index2,
                    matrix_index,
                ] = ForwardDiff.partials(dual_value)[ε_index]
            end
        end
    end
end
...
function update_jacobian!(alg::AutoDenseJacobian, cache, Y, p, dtγ, t)
    ...
    # Set column_lu_factors to dtγ * J - I, where J is the dense Jacobian.
...
function invert_jacobian!(::AutoDenseJacobian, cache, ΔY, R)
    ...
    scalar_level_pairs = scalar_level_pair_iterator(ΔY)

    # Copy values from R into column_lu_vectors.
    ClimaComms.@threaded device begin
        for (vector_index, column_index) in enumerate(column_indices),
            (scalar_level_index, (name_index, level)) in scalar_level_pairs

            value = unrolled_applyat(name_index, scalar_names) do name
    ...
    # Copy values from column_lu_vectors into ΔY.
    ClimaComms.@threaded device begin
        for (vector_index, column_index) in enumerate(column_indices),
            (scalar_level_index, (name_index, level)) in scalar_level_pairs
            ...
            unrolled_applyat(name_index, scalar_names) do name
                ...
                @inbounds point(field, level, column_index...)[] = value
            end
        end
    end
end
