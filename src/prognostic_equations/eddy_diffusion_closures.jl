...
Calculates the surface flux of Turbulent Kinetic Energy (TKE), a C3 vector used by 
...
Details:
- `c_k`: A dimensionless coefficient (`tke_surface_flux_coeff`) obtained from
         `turbconv_params`.
...
- `ρa_sfc`: Air density (area-fraction weighted) at the surface [kg/m^3].
...
- A `ClimaCore.Geometry.C3` vector representing the TKE flux normal to the surface.
  The flux is directed along the surface upward normal.
"""
function surface_flux_tke(
    ...
    # Extract the dimensionless coefficient c_k from parameters
    c_k = CAP.tke_surface_flux_coeff(turbconv_params) # Placeholder name, adjust as needed

    # Calculate the magnitude of the TKE flux: c_k * ρ * ustar^3
    tke_flux_magnitude = c_k * ρa_sfc * ustar^3
    ...
    c3_unit_vector = C3(unit_basis_vector_data(C3, surface_local_geometry))

    # Return the TKE flux vector
    return tke_flux_magnitude * c3_unit_vector
end
...
- `ᶜN2_eff`: Effective squared Brunt-Väisälä frequency [1/s^2].
...
function mixing_length(
    ...
    a_pd = c_m * (2 * ᶜstrain_rate_norm - ᶜN2_eff / ᶜPr) * sqrt_tke_pos
    ...
    N_eff_sq = max(ᶜN2_eff, FT(0)) # Use N^2 only if stable (N^2 > 0)
...
"""
    turbulent_prandtl_number(params, ᶜN2_eff, ᶜstrain_rate_norm)
...
function turbulent_prandtl_number(params, ᶜN2_eff, ᶜstrain_rate_norm)
    ...
    ᶜRi_grad = ᶜN2_eff / ᶜshear_term_safe
...
function blend_scales(method::AbstractScaleBlending, l::SA.SVector, turbconv_params)
    error("Unimplemented blending method")
end
...
"""
    eddy_viscosity_diffusivity(turbconv_params, tke, mixing_length, prandtl_nvec)

Calculates the eddy viscosity (K_u) for momentum and eddy diffusivity (K_h) for 
scalars based on the turbulent kinetic energy (TKE), the mixing length, and the 
