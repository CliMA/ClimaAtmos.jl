...
"""
    rrtmgp_solver(params, context, radiation_mode = ClearSkyRadiation(); kwargs...)
...
# Keyword Arguments
  ...
  - `toa_flux`: irradiance of sun [W/m²] (required); the incoming
    direct shortwave radiation is given by `toa_flux .* cos_zenith`
...
# Seed an RRTMGP input buffer from `dict[name]`. For `center_`/`face_` fields only the
# physical domain is written; the isothermal boundary layer (if any) is filled by RRTMGP
# inside `update_fluxes!`. The buffers are later read/written through the solver getters.
