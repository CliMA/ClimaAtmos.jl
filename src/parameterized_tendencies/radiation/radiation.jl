...
function rrtmgp_model_kwargs(
    ...
    atmospheric_gas_params = CAP.atmospheric_gas_params(params)
    ...
    center_volume_mixing_ratio_o3 =
        :o3 in Symbol.(lowercase.(time_varying_trace_gases)) ? NaN :
        Fields.field2array(idealized_ozone.(ᶜz))

    atmospheric_gas_names = (
        :co2,
        :n2o,
        :co,
        :ch4,
    ...
    atmospheric_gas_vmr_names =
        map(gas_name -> Symbol(:volume_mixing_ratio_, gas_name), atmospheric_gas_names)
    atmospheric_gas_vmrs = map(atmospheric_gas_names) do gas_name
        if gas_name in Symbol.(lowercase.(time_varying_trace_gases))
            NaN
        else
            getfield(atmospheric_gas_params, Symbol(gas_name, :_fixed_value))
        end
    end
    kwargs = (;
        kwargs...,
        ...
        center_volume_mixing_ratio_o3,
        NamedTuple{atmospheric_gas_vmr_names}(atmospheric_gas_vmrs)...,
    )
