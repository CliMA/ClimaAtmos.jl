        ...
        for (rrtmgp_aerosol_name, prescribed_aerosol_name) in prescribed_aerosol_names_pair
            ...
            if prescribed_aerosol_name in
               propertynames(p.tracers.prescribed_aerosols_field)
                ...
                # TODO: prognostic aerosols (Y.c.ρ<name>) are not yet supplied to
                # RRTMGP. A bin that is prognostic-only is absent from
                ...
                # Read prognostic aerosol mass into ᶜaero_conc here once supported.
                @. ᶜaero_conc = 0
            end
        end
    end
