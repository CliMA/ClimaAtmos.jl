...
const SEA_SALT_BIN_BOUNDS = (
    ...
    (0.1, 0.5),   # μm, rep size 0.316 μm, SSLT02
    (0.5, 1.5),   # μm, rep size 1.119 μm, SSLT03
    (1.5, 5.0),   # μm, rep size 2.818 μm, SSLT04
    (5.0, 10.0),   # μm, rep size 7.772 μm, SSLT05
)
...
Compute the upward sea salt number flux (particles m⁻² s⁻¹) per bin
using [Gong2003](@cite). SST correction from [Jaegle2011](@cite).
"""
...
store them in `p.tracers`. u_10 dependence extrapolated from u* and
lowest model level wind using MOST and `surface_fluxes_params`.
...
Apply surface emission tendencies for interactive sea salt bins (ρSSLT01 …).

Reads per-bin fluxes from `p.tracers.interactive_aerosols_field`, pre-computed by
`set_sea_salt_emission_flux!`. Flux is applied as a bottom boundary condition using
`boundary_tendency_scalar`, localizing it to the lowest model layer.
"""
...
Apply deposition tendencies to all interactive sea salt bins.

Currently implemented as a simple exponential decay. Separate wet and dry deposition
tendencies are forthcoming
"""
