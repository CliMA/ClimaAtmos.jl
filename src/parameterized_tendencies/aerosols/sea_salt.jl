...
function set_sea_salt_emission_flux!(Y, p)
    prognostic_aerosol_names = _aerosol_names(p.atmos.prognostic_aerosols)
    isempty(prognostic_aerosol_names) && return
    ...
    for (bin_index, name) in enumerate(prognostic_aerosol_names)
