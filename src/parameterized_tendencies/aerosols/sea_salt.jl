...
`p.precomputed.sfc_conditions` forced non-negative. Returns `u_z`.
"""
function set_wind_at_height!(u_z, z, Y, p)
    ...
    z = FT(z)
    @. u_z = wind_at_height(z, ustar, obukhov_length, sfp)
    return u_z
end
...
@generated seasalt_state_names(::PrognosticSeaSalt{names}) where {names} =
...
Write each bin's upward sea salt mass flux (physical magnitude
[kg m⁻² s⁻¹], stored as the covariant component of a surface-normal
vector) — the
...
function set_sea_salt_surface_fluxes!(
    ...
    L_uninitialized = FT(1e-4)
    @. u_10 = ifelse(obukhov_length == L_uninitialized, zero(u_10), u_10)

    u_ref = ap.ssa_u_ref
    wind_exp = ap.gong_wind_exp
    wind_factor = u_10
    @. wind_factor = (u_10 / u_ref)^wind_exp
...
function aerosol_emission_tendency!(Yₜ, Y, p, t, seasalt::PrognosticSeaSalt)
    ...
    fluxes = p.tracers.seasalt_sfc_fluxes

    MatrixFields.unrolled_foreach(seasalt_state_names(seasalt)) do ρχ_name
