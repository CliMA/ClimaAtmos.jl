...
function compute_beres_convective_heating!(Y, p, ᶜN)
    ...
    Operators.column_reduce!(
        ...
        h_env = max(z_top - z_bot, eps(FT))
        dz_g = h_env / FT(N_BERES_PROFILE - 1)
        Q_pos = max(Q_ic, FT(0))
        newvals = ntuple(ng_val) do i
            z_i = z_bot + FT(i - 1) * dz_g
    ...
    let beres = gw_beres_source, cgrid = gw_c, ncv = gw_ncval
        b0(active, u, Q0, h, N, zbot, ztop, gprof, w) =
            _beres_launch_spectrum_or_zero(
                active, cgrid, u, Q0, h, N, zbot, ztop - zbot, gprof, w, beres,
                ncv,
            )
        @. gw_beres_B0_u = b0(
...
function waveforcing_column_accumulate!(
    ...
    Operators.column_accumulate!(
        ...
        # Beres slot 8 is the precomputed launch spectrum B0 (NTuple{nc}); for the
        # AD99 specialization it is a dead zero tuple. The old per-column scalars
        # (Q0/h/u_heat/N at slots 9–12) are no longer consumed — B0 already folds
        # them in (shape mode, steady β_therm + β_mech) from compute_tendency!.
        beres_B0 =
            MODE == :beres ? inp[8] : ntuple(_ -> zero(FT1), gw_ncval)
...
@inline function _beres_gs(
    ...
    for i in 2:ng
        z_b = z_bot + FT(i - 1) * dz
        g_b = gprofile[i]
        acc += _beres_sine_segment(z_a, g_a, z_b, g_b, m)
        z_a = z_b
        g_a = g_b
    end
    return acc
...
function _beres_launch_spectrum(
    ...
    # Beres-G uses the gathered profile only when the flag is on AND a profile is
    # supplied (the wave_source wrapper passes `nothing` ⇒ always half-sine).
