...
function compute_precipitation_sinks!(
    ...
    @. Sᵖ = -min(
        limit(qₚ(ρqᵣ, ρ), dt),
        -CM1.evaporation_sublimation(rps..., qₜ(thp, ts), qₚ(ρqᵣ, ρ), ρ, Tₐ(thp, ts)),
    )
    @. Sqₜᵖ -= Sᵖ
    @. Sqᵣᵖ += Sᵖ
    @. Seₜᵖ -= Sᵖ * (Iₗ(thp, ts) + Φ)
    ...
    @. Sᵖ = CM1.evaporation_sublimation(sps..., qₜ(thp, ts), qₚ(ρqₛ, ρ), ρ, Tₐ(thp, ts))
    @. Sᵖ = ifelse(
        Sᵖ > FT(0),
