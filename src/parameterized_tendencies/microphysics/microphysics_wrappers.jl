...
function compute_1m_precipitation_tendencies!(
    ...
    @. mp_result = apply_1m_tendency_limits(mp_result, thp, qₜ, qₗ, qᵢ, qᵣ, qₛ, dt)
    @. Sqₗᵐ = mp_result.dq_lcl_dt
...
"""
    microphysics_tendencies_quadrature_0m(
        ...
        ρ, T_mean, q_tot_mean, T′T′, q′q′, T′q′,
    )
...
@inline function microphysics_tendencies_quadrature_0m(
    ...
    # Limit covariances for physical validity
    σ_q, σ_T, corr = limit_covariances(q′q′, T′T′, T′q′, q_tot_mean, SG_quad)
...
Call sites must convert θ-based covariances to T-based covariances using the chain rule:
```julia
∂T_∂θ = ... # (∂T/∂θ_liq_ice) computed at grid mean state
T′T′ = (∂T_∂θ)² × θ′θ′
T′q′ = ∂T_∂θ × θ′q′
```
This conversion is done once per grid cell, avoiding θ→T conversion at each quadrature point.
"""
