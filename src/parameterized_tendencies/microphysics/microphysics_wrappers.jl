...
before calling the layer-profile [`integrate_over_sgs`](@ref) overload:
...
@inline function microphysics_tendencies_1m_sgs_row(
    ...
    # Vertical-profile LogNormal transformed-slope handling is centralized in
    # `_integrate_over_sgs_profile_rosenblatt`, so wrappers always pass raw
    # physical-space gradients here.
    gq_dn = grad_q_dn
    gq_up = grad_q_up
...
@inline function microphysics_tendencies_1m(
    scheme, sgs_quad::SGSQuadrature{N, A, W, D}, cmp, thp, ρ, T, q_tot_nonneg,
    ...
    # and the layer-profile [`integrate_over_sgs`](@ref) overload.
    # (Dispatch excludes the wrong types.)
    ...
    # Vertically resolved 1M uses the layer-profile `integrate_over_sgs` overload at the
