...
function cloud_sources(
    ...
    qᵥ = qₜ - qₗ - qᵢ - qᵣ - qₛ
    qₛₗ = TD.q_vap_saturation_from_density(thp, T, ρ, TD.saturation_vapor_pressure(thp, T, TD.Liquid()))
...
function cloud_sources(
    ...
    qₛᵢ = TD.q_vap_saturation_from_density(thp, T, ρ, TD.saturation_vapor_pressure(thp, T, TD.Ice()))
