...
function precipitation_tendency!(
    ...
    @. Yₜ.c.ρq_liq += Y.c.ρ * (ᶜSqₗᵖ⁰ + ᶜSqₗᵖ)
    @. Yₜ.c.ρq_ice += Y.c.ρ * (ᶜSqᵢᵖ⁰ + ᶜSqᵢᵖ)
...
function precipitation_tendency!(
    ...
    # Update from environment precipitation sources
    # and the grid mean precipitation sinks
    @. Yₜ.c.ρq_liq += ᶜρa⁰ * ᶜSqₗᵖ⁰ + Y.c.ρ * ᶜSqₗᵖ
    @. Yₜ.c.ρq_ice += ᶜρa⁰ * ᶜSqᵢᵖ⁰ + Y.c.ρ * ᶜSqᵢᵖ
