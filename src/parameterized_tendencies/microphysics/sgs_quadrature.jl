...
- `default_corr::FT`: Default T-q correlation used when covariance is zero.
  Typical atmospheric values are strongly negative (-0.9 to -0.7).
...
# Constructors
    ...
    SGSQuadrature(FT; quadrature_order=2, distribution=GaussianSGS(), default_corr=-0.9, T_min=150.0)

Create an `SGSQuadrature` with the specified floating-point type `FT`,
quadrature order, distribution type, default correlation, and minimum temperature.

The default correlation is used in `limit_covariances` when the computed
correlation is near zero (indicating missing covariance data). This accounts
for the typical strong negative correlation between T and q_tot in the
atmosphere (moist adiabatic processes cause temperature to decrease as
moisture increases).
...
struct SGSQuadrature{N, A, W, D <: AbstractSGSDistribution, FT} <: AbstractSGSamplingType
    ...
    w::W             # quadrature weights
    dist::D          # distribution type
    default_corr::FT # default T-q correlation when covariance is zero
...
Applies three constraints:
1. ``\\sigma_q`` is bounded to prevent negative ``q_{tot}``:
   ``\\sigma_q \\leq -q_{mean} / (\\sqrt{2} \\chi_1)``
2. Cauchy-Schwarz inequality: ``|\\rho| \\leq 1``
3. Uses `quad.default_corr` when variances are too small to compute
   a meaningful correlation (σ_q < ε or σ_T < ε)
...
@inline function limit_covariances(q′q′, T′T′, T′q′, q_mean, quad)
    ...
    # Check if variances are too small to compute meaningful correlation
    # This indicates missing or negligible covariance data
    variance_too_small = (σ_q < eps_q) | (σ_T < eps_T)

    # Enforce Cauchy-Schwarz inequality: |corr| ≤ 1
    _corr = T′q′ / max(σ_q, eps_q)
    corr_computed = clamp(_corr / max(σ_T, eps_T), -one(FT), one(FT))

    # Use default correlation when variances are too small to compute
    # a meaningful correlation. The default is typically negative since
    # T and q_tot are strongly anti-correlated in the atmosphere.
    corr = ifelse(variance_too_small, quad.default_corr, corr_computed)
...
"""
    integrate_over_sgs(f, ::GridMeanSGS, μ_q, μ_T, q′q′, T′T′, T′q′)
...
avoiding the need to extract FT from the space. Covariances are ignored.
"""
@inline function integrate_over_sgs(f, ::GridMeanSGS, μ_q, μ_T, q′q′, T′T′, T′q′)
    return f(μ_T, μ_q)
end
