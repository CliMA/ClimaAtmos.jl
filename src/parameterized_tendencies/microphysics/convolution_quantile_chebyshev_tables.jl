# Chebyshev support tables for convolution quantiles:
# - GaussianSGS: static, precomputed degree-12 coefficients in mapped log10(η).
# - LogNormalSGS: reuses the same 1D coefficient tables in transformed ln(q)-space.
# Unsupported `(N_GL, i_node)` combinations error explicitly.
# Regeneration scripts must keep table shape/ranges consistent.
...
"""LogNormalSGS Chebyshev in ln(q)-space reuses the Gaussian 1D coefficient tables."""
@inline function chebyshev_convolution_coeffs(::LogNormalSGS, ::Type{FT}, N_GL::Int, i_node::Int) where {FT}
    return chebyshev_convolution_coeffs(GaussianSGS(), FT, N_GL, i_node)
end
