...
These are diffused with the unscaled eddy diffusivity `K_h`, both in the
tendencies and in the implicit Jacobian, whereas the sedimenting species use
`K_h` rescaled by `α_vert_diff_tracer`.
"""
passive_gs_tracer_names(Y) =
    unrolled_filter(
        name -> !(
