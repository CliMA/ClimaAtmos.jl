...
function weather_model_data_path(start_date, start_time)
    # get data path
    ic_data_path = joinpath(@clima_artifact("weather_model_ic"), "init", "era5_init_$(start_date)_$(start_time).nc")
    raw_data_path = joinpath(@clima_artifact("weather_model_ic"), "raw", "era5_raw_$(start_date)_$(start_time).nc")
    ...
    if !isfile(ic_data_path)
        ...
        if !isfile(raw_data_path)
            ...
            error("Source file $(raw_data_path) does not exist. Please run `python get_initial_conditions.py --output-dir $(@clima_artifact("weather_model_ic"))/raw --date $(day) --time $(time)` to download the data.")
        end
        # get target levels - TODO: make this more flexible
        target_level_path = joinpath(@clima_artifact("weather_model_ic"), "target_levels.txt")
