...
# broken. Applied for RCEMIPI only. Seeded from coordinates → reproducible and
# decomposition-independent.
#
# noise layers are set by the prescribed RCEMIP zmesh, the 5 lowest layers
const NOISE_LAYER_HEIGHTS = (18.5, 74.5, 153.0, 241.0, 341.5)  # m, lowest 5 centers
const NOISE_AMPLITUDES = (0.1, 0.08, 0.06, 0.04, 0.02)      # K, layers 1..5
const NOISE_TOL = 1.0    # m

function rcemipi_thermal_noise(coords)
    z = coords.z
    FT = typeof(z)
    k = findfirst(zc -> abs(z - zc) < NOISE_TOL, NOISE_LAYER_HEIGHTS)
    k === nothing && return zero(FT)
    return FT(NOISE_AMPLITUDES[k]) * (2 * rand(FT) - 1)  # uniform in [-A, A] for A amplitude
end
