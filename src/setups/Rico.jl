...
subsidence_forcing(::Rico, ::Type{FT}) where {FT} = APL.Rico_subsidence(FT)

large_scale_advection_forcing(::Rico, ::Type{FT}) where {FT} =
    (; prof_dTdt = APL.Rico_dTdt(FT), prof_dqtdt = APL.Rico_dqtdt(FT))


coriolis_forcing(::Rico, ::Type{FT}) where {FT} =
    (;
