...
"""
function vars_state(balance_law::LinearThreeDimensionalDryCompressibleEulerWithTotalEnergy, aux::Auxiliary, FT)
    @vars begin
        x::FT
        y::FT
        z::FT
...
vars_state(::LinearThreeDimensionalDryCompressibleEulerWithTotalEnergy, ::DryReferenceState, ::Auxiliary, FT) =
    @vars(T::FT, p::FT, ρ::FT, ρu::SVector{3, FT}, ρe::FT)

vars_state(::LinearThreeDimensionalDryCompressibleEulerWithTotalEnergy, ::NoReferenceState, ::Auxiliary, FT) =
    @vars()

function vars_state(::LinearThreeDimensionalDryCompressibleEulerWithTotalEnergy, ::Prognostic, FT)
    @vars begin
        ρ::FT
        ρu::SVector{3, FT}
        ρe::FT
...
function init_state_auxiliary!(
    ::LinearThreeDimensionalDryCompressibleEulerWithTotalEnergy,
    ::NoReferenceState,
    state_auxiliary,
    geom,
) end
...
"""
    Utils
"""
function altitude(balance_law::LinearThreeDimensionalDryCompressibleEulerWithTotalEnergy, ::SphericalOrientation, geom)
    return norm(geom.coord) - balance_law.parameters.a
end

function altitude(::LinearThreeDimensionalDryCompressibleEulerWithTotalEnergy, ::FlatOrientation, geom)
