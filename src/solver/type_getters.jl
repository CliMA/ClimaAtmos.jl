...
get_jacobian(ode_algo, Y, atmos, parsed_args, output_dir) =
    if ode_algo isa Union{CTS.IMEXAlgorithm, CTS.RosenbrockAlgorithm}
        ...
        jacobian_algorithm = if debug_manual_jacobian
        ...
        elseif use_auto_jacobian
            # AutoDenseJacobian() # The LU factorization for this is very slow.
            AutoSparseJacobian(manual_jacobian_algorithm)
        else
            manual_jacobian_algorithm
        end
        @info "Jacobian algorithm: $(summary_string(jacobian_algorithm))"
        Jacobian(jacobian_algorithm, Y, atmos, output_dir)
    else
        nothing
    end
