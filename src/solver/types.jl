...
#
# Each constructor takes (config::AtmosConfig, ...) and returns the
# corresponding group struct, consolidating the YAML→typed-object translation
# that used to be split across `model_getters.jl` helpers.
#
# Large dispatch helpers (radiation modes, gravity wave parameter destructure,
# external-forcing model, etc.) remain in `model_getters.jl`; they're called
# only from these constructors.
