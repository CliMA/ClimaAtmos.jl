...
"""
    maybe_resolve_and_acquire_artifacts(input_str::AbstractString, context::ClimaComms.AbstractContext)
...
function maybe_resolve_and_acquire_artifacts(
    ...
    context::ClimaComms.AbstractContext,
)
    ...
    if isnothing(matched)
        ...
        return joinpath(@clima_artifact(artifact_name, context), other_path)
    end
end

function maybe_resolve_and_acquire_artifacts(
    input,
    _::ClimaComms.AbstractContext,
)
    return input
end

"""
    config_with_resolved_and_acquired_artifacts(input_str::AbstractString, context::ClimaComms.AbstractContext)

Substitutes strings of the form `artifact"name"/something/else` with the actual
artifact path.
"""
