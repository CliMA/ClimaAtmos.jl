...
function get_external_forcing_model(
    ...
    elseif external_forcing == "ReanalysisTimeVarying"
        external_forcing_file = get(parsed_args, "external_forcing_file", nothing)
        if isnothing(external_forcing_file)
            ...
            @assert isfile(external_forcing_file) "Specified external_forcing_file does not exist: $(external_forcing_file)"
        end
