...
function hydrostatic_pressure_profile(;
    ...
    function ρ_from_profile(p, z, ::Nothing, θ::FunctionOrSpline, ::Nothing)
        T_val = TD.air_temperature(thermo_params, TD.pθ(), p, oftype(p, θ(z)))
...
for IC in (:GABLS,)
    ...
    @eval function (initial_condition::$IC)(params)
        ...
        function local_state(local_geometry)
            (; z) = local_geometry.coordinates
            T = TD.air_temperature(thermo_params, TD.pθ(), p(z), θ(z))
            return LocalState(;
                params,
                geometry = local_geometry,
                T = T,
...
function (initial_condition::GATE_III)(params)
    ...
    p = hydrostatic_pressure_profile(; thermo_params, p_0, T = T_profile, q_tot = q_tot_profile)
    u = APL.GATE_III_u(FT)
...
function (initial_condition::PrecipitatingColumn)(params)
    ...
    function local_state(local_geometry)
        ...
        T = TD.air_temperature(thermo_params, TD.pθ_li(), p(z), θ(z), q_tot_z, q_liq_z, q_ice_z)
        return LocalState(;
            params,
            geometry = local_geometry,
            T = T,
