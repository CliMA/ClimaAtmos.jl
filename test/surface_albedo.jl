...
@testset "set_surface_albedo!" begin
    ...
    @testset "ConstantAlbedo" begin
        ...
        @test all(RRTMGP.direct_sw_surface_albedo(p.radiation.rrtmgp_solver) .== FT(0.38))
        @test all(RRTMGP.diffuse_sw_surface_albedo(p.radiation.rrtmgp_solver) .== FT(0.38))
    end
    ...
    @testset "RegressionFunctionAlbedo" begin
        ...
        @test !isnan(sum(RRTMGP.direct_sw_surface_albedo(p.radiation.rrtmgp_solver)))
        @test !isnan(sum(RRTMGP.diffuse_sw_surface_albedo(p.radiation.rrtmgp_solver)))
    end
    ...
    @testset "CouplerAlbedo" begin
        ...
        @test all(isnan.(RRTMGP.direct_sw_surface_albedo(p.radiation.rrtmgp_solver)))
        @test all(isnan.(RRTMGP.diffuse_sw_surface_albedo(p.radiation.rrtmgp_solver)))
