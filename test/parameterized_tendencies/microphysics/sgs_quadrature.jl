...
@testset "SGS Quadrature" begin
    ...
    @testset "SGSQuadrature Struct" begin
        for FT in (Float32, Float64)
            @testset "FT = $FT" begin
                # Default (Gaussian, order 2)
                quad = ClimaAtmos.SGSQuadrature(FT)
                @test ClimaAtmos.quadrature_order(quad) == 2
                @test quad.dist isa ClimaAtmos.GaussianSGS
                @test quad.default_corr ≈ FT(-0.9)
    ...
    @testset "Default Correlation Fallback" begin
        for FT in (Float32, Float64)
            @testset "FT = $FT" begin
                custom_default = FT(-0.85)
                quad = ClimaAtmos.SGSQuadrature(FT; default_corr = custom_default)

                # When variances are too small (σ < ε), use default correlation
                # Test 1: Zero q variance → use default
                σ_q, σ_T, corr = ClimaAtmos.limit_covariances(
                    FT(0), FT(1.0), FT(0), FT(0.01), quad,
                )
                @test corr == custom_default
                @test σ_q == FT(0)

                # Test 2: Zero T variance → use default
                σ_q2, σ_T2, corr2 = ClimaAtmos.limit_covariances(
                    FT(1e-6), FT(0), FT(0), FT(0.01), quad,
                )
                @test corr2 == custom_default
                @test σ_T2 == FT(0)

                # Test 3: Both variances zero → use default
                σ_q3, σ_T3, corr3 = ClimaAtmos.limit_covariances(
                    FT(0), FT(0), FT(0), FT(0.01), quad,
                )
                @test corr3 == custom_default

                # Test 4: Both variances positive → use computed correlation
                # Large positive covariance to ensure computed corr is well-defined
                q′q′ = FT(1e-6)
                T′T′ = FT(1.0)
                T′q′ = FT(0.5e-3)  # Positive covariance
                σ_q4, σ_T4, corr4 = ClimaAtmos.limit_covariances(
                    q′q′, T′T′, T′q′, FT(0.01), quad,
                )
                # Computed correlation should be positive (same sign as covariance)
                @test corr4 > FT(0)
                @test corr4 != custom_default

                # Test 5: Verify default is used with typical default_corr
                quad_default = ClimaAtmos.SGSQuadrature(FT)  # default_corr = -0.9
                _, _, corr5 = ClimaAtmos.limit_covariances(
                    FT(0), FT(1.0), FT(0), FT(0.01), quad_default,
                )
                @test corr5 == FT(-0.9)
            end
        end
    ...
    @testset "Quadrature Integration" begin
        for FT in (Float32, Float64)
            @testset "FT = $FT" begin
                ...
                result =
                    ClimaAtmos.integrate_over_sgs(f_const, quad, μ_q, μ_T, q′q′, T′T′, T′q′)
                @test result ≈ FT(1) atol = FT(1e-6)

                # Integrate identity for T → should return mean T
                f_T(T, q) = T
                result_T =
                    ClimaAtmos.integrate_over_sgs(f_T, quad, μ_q, μ_T, q′q′, T′T′, T′q′)
                @test result_T ≈ μ_T atol = FT(1.0)

                # NamedTuple return
                f_nt(T, q) = (; val1 = T, val2 = q)
                result_nt =
                    ClimaAtmos.integrate_over_sgs(f_nt, quad, μ_q, μ_T, q′q′, T′T′, T′q′)
            ...
            @testset "FT = $FT" begin
                ...
                @testset "Zero Variance = Grid Mean" begin
                    ...
                    result_quad = microphysics_tendencies_quadrature(
                        ...
                        quad, mp, tps, ρ, p_c,
                        T_mean, q_tot_mean, q_lcl_mean, q_icl_mean, q_rai, q_sno,
                        FT(0), FT(0), FT(0),  # Zero variances/covariance
                    )
                ...
                @testset "Variance Effect" begin
                    ...
                    result_var = microphysics_tendencies_quadrature(
                        ...
                        quad, mp, tps, ρ, p_c,
                        T_mean, q_tot_mean, q_lcl_mean, q_icl_mean, q_rai, q_sno,
                        FT(4.0), FT(1e-5), FT(1e-3),  # Non-zero variances
                    )
        ...
        for FT in (Float32, Float64)
            @testset "FT = $FT" begin
                ...
                @testset "0M quadrature: dq_tot_dt ≤ 0" begin
                    ...
                    # Zero covariances (grid-mean evaluation)
                    ...
                    # With covariances (SGS fluctuations)
                ...
                @testset "1M quadrature: sign consistency" begin
                    ...
                    # With zero covariances, quadrature should match direct BMT
                    ...
                    # With non-zero covariances, should still be finite
