...
@testset "Geometric SGS variance parameters" begin
    ...
    for FT in (Float32, Float64)
        ...
        # the temperature channel shares c_g by default; a species is half uniform
        # under the blended distribution by default
        @test CAP.sgs_variance_geometric_T_factor(tc) == FT(1)
        @test CAP.sgs_condensate_uniform_fraction(tc) == FT(0.5)
    end
  ...
  value = 1.0
  type = "float"
  [sgs_variance_geometric_T_factor]
  value = 2.0
  type = "float"
  [sgs_condensate_uniform_fraction]
  value = 0.25
  type = "float"
  """,
        )
        ...
        @test CAP.sgs_variance_geometric_T_factor(tc) == 2.0
        @test CAP.sgs_condensate_uniform_fraction(tc) == 0.25
    end
end
