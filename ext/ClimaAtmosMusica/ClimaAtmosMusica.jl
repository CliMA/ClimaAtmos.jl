...
function ClimaAtmos.update_chemistry!(
    ...
    for i in 1:n_cells
        ...
        concs = Dict(
            "q_gas_A" => Float64(parent(getproperty(Y.c, Symbol(:ρ, :q_gas_A)))[i])/mw_A,
            "q_gas_B" => Float64(parent(getproperty(Y.c, Symbol(:ρ, :q_gas_B)))[i])/mw_B,
            "q_gas_AB" => Float64(parent(getproperty(Y.c, Symbol(:ρ, :q_gas_AB)))[i])/mw_AB,
        )
        ...
        # end
        parent(getproperty(Y.c, Symbol(:ρ, :q_gas_A)))[i] = Float64(only(updated["q_gas_A"]))*mw_A
        parent(getproperty(Y.c, Symbol(:ρ, :q_gas_B)))[i] = Float64(only(updated["q_gas_B"]))*mw_B
        parent(getproperty(Y.c, Symbol(:ρ, :q_gas_AB)))[i] = Float64(only(updated["q_gas_AB"]))*mw_AB
    end
    return nothing
end
