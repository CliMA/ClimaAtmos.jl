...
"""
    plot_edmf_N!(emdfparams, ax)

Base plotting function for LWP vs N on an Axis object.
"""
function plot_edmf_N!(
    edmfparams::Vector{EDMFParams},
    ax::CairoMakie.Axis,
    is_1M::Bool,
)
    ...
    for (out, prescribed_N) in all_paths
        ...
        if is_1M
            ...
            N_data = fill(prescribed_N, length(lwp_data)) * 1e-6 # Convert to cm^-3.
        else
            # Number concentration.
            N = ClimaAnalysis.get(
                simdir;
            ...
            N_slice = ClimaAnalysis.slice(N, x = 0.0, y = 0.0)
            N_skip = ClimaAnalysis.window(N_slice, "time", left = 7200)
            N_data = [mean(N_skip.data, dims = 2)...] * 1e-6 # Convert to cm^-3.
        end
        ...
        CairoMakie.scatter!(ax, lwp_data[1], N_data[1], color = :grey)
    end
end
...
function decide_plotting!(
    ...
    if is_time
        plot_edmf_timeseries!(edmfparams, ax)
    else
        plot_edmf_N!(edmfparams, ax, is_1M)
    end
end
...
Remove simulations that ended early or with no stable clouds.
"""
...
"""
    filter_runs(edmfparams)

Filters a vector of simulation outputs based on the conditions in the keep 
function. 
"""
