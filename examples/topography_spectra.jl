...
function generate_spaces(;h_elem=16, diffiter=32)
   ...
   elev_from_file = 
      SpaceVaryingInputs.SpaceVaryingInput(
        AA.earth_orography_file_path(;context=comms_ctx), "z", h_space
      )
...
function remap_to_nc(var, hcoords)
...
function gen_spectra(var)
    ...
    power_spectrum = dropdims(sum(spectrum_data, dims=1), dims=1)[begin:(end-1),:]
...
function generate_all_spectra(;h_elem=32)
    ...
    for ii in (0, 4, 8, 16, 32, 64, 128, 256)
        ...
        fig = Plots.plot!(sph_wn[2:end], log.(psd)[2:end];lw=2, 
                          xlabel="Spherical wavenumber", ylabel="log(elevation power spectra)", label="$(ii)×", xaxis=:log, 
                          xticks=([2 4 8 16 32 64 128],["2", "4", "8", "16", "32", "64", "128"]))
    end
    fig = Plots.plot!(; legend=:bottomleft, grid=:off, fontsize=20, title="Biharmonic smoothing")
    return fig, var
end
