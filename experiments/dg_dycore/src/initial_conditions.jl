...
function initial_state_fddg(m::DGModel{FT}) where {FT}
    ...
    #
    # ρw = 0 is the physically-balanced IC over terrain: a coordinate-
    # tangent IC (w = uₕ·level-slope) injects several m/s of spurious
    # vertical motion through the column and crashes within a few HEVI
    # steps; with ρw = 0 the surface impermeability adjustment is absorbed
    # by the implicit acoustics. If a tangent IC is ever needed, decay it
    # near the surface — never column-wide.
