...
# ---------------------------------------------------------------------------
# Minimal setup to call `set_cloud_fraction!` at the bottom of this file.
#
...
# (invoked by `build_cache`) already runs, so building the cache below
# exercises it once; the call at the bottom re-runs just that step. It reads
# only the thermodynamic / SGS-covariance parts of `p.precomputed` and never
# reads the radiation, aerosol, trace-gas, gravity-wave, tracer, or external-
# forcing parts of `p`. Those are also the *expensive* part of model setup
# (RRTMGP compilation + lookup tables, ETOPO topography load + spectral
...
# place. None of these knobs affect which branch cloud fraction takes.
...
# so the precomputed quantities `set_cloud_fraction!` reads are populated here.
# The aerosol / trace-gas tuples are now empty, so the tracer cache is a stub.
...
CA.set_cloud_fraction!(Y, p, microphysics_model, cloud_model)
BenchmarkTools.@benchmark CUDA.@sync CA.set_cloud_fraction!(
           $Y,
           $p,
           $microphysics_model,
           $cloud_model,
