...
# ---------------------------------------------------------------------------
# Minimal setup to call `set_sgs_moments_and_cloud_fraction!` at the bottom of
# this file.
#
...
# building the cache below exercises it once; the call at the bottom re-runs
# just that step. It reads only the thermodynamic / SGS-covariance parts of
# `p.precomputed` and never reads the radiation, aerosol, trace-gas, gravity-
# wave, tracer, or external-forcing parts of `p`. Those are also the
# *expensive* part of model setup (RRTMGP compilation + lookup tables, ETOPO
# topography load + spectral smoothing, aerosol/trace-gas data ingestion, Beres
# gravity-wave tables), so we strip them from the config below *before*
# building anything. `ClimaAtmosParameters`/`get_atmos` then never load their
# parameter sets, `build_cache` constructs trivial stubs in their place, and
...
# un-needed parameter sets, data files, and caches are never constructed in the
# first place. None of these knobs affect which branch the routine takes.
...
CA.set_sgs_moments_and_cloud_fraction!(Y, p)
BenchmarkTools.@benchmark CUDA.@sync CA.set_sgs_moments_and_cloud_fraction!($Y, $p)
