...
Gaussian subgrid variability, quadrature design, and a bounded layer marginal}}
\author{}
\date{\today}
...
Using GH nodes as if the marginal along an axis were standard normal when it is actually a \textbf{bounded Gaussian--uniform convolution} misplaces mass and can grossly distort expectations when $g$ grows in the tails.

\paragraph{CliMA calibration: mixture marginal vs.\ segment kernels in \emph{this} note.}
This document derives the \textbf{closed-form segment objects} and the assembled \profileros\ quadrature (Gaussian--uniform marginals, $\Psi_\nu$ tails, Mills identities, stable \texttt{erf}/\texttt{erfc} algebra) that appear in Python \texttt{profile\_rosenblatt\_two\_half\_cell.py} and Julia \texttt{subgrid\_layer\_profile\_quadrature.jl}.
Separately, the CliMA variance-adjustments stack defines a \textbf{two-half} mixture marginal $p_{\mathrm{mix}}(T,q)$ as a uniform $z$-average of DN/UP bivariate normals; see \path{ORACLE_SGS.md} in the same repository area.
\textbf{Calibration Python} evaluates that definition via \texttt{mixture\_pdf\_grid}: a Riemann sum over $z$ of exact multivariate-normal PDFs.
Those are \textbf{different layers}: the derivations here are \textbf{not} ``missing''---they power two-half profile cubature---while \texttt{mixture\_pdf\_grid} is the \textbf{specified reference evaluator} for $p_{\mathrm{mix}}$ unless an alternate implementation is proved equivalent in tests.
\textbf{Documentation error} (not a missing proof) is to conflate ``\texttt{mixture\_pdf\_grid} does not call routine X'' with ``nothing was derived.''
...
\begin{enumerate}
  ...
  \item We present the \profileros\ scheme as \textbf{Rosenblatt conditioning} in coordinates aligned with the mean gradient~$\mathbf{d}$: outer quadrature for a Gaussian marginal, inner \textbf{inverse-CDF} (probability-space GL) for the bounded inner marginal at each outer node.
...
Sections~\ref{sec:grad-align-coords}--\ref{sec:brent} build the inner marginal and quantiles.
...
$\eta$ & $s_{u\mid v}/L$ \\
$F,\,f,\,f',\,f''$ & CDF/PDF of inner marginal and derivatives (\S\ref{sec:pill-marginal}) \\
\midrule
...
\profileros & Rosenblatt factorization along $\mathbf{d}$ with bounded inner marginal ($\mathcal{O}(N^2)$) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Path half-width $L=H$.}
The coordinate~$u$ along the mean direction is normalized so that one unit of~$u$ corresponds to one unit of physical~$z$ along the layer; the uniform factor in the inner marginal has support $[-L/2,L/2]=[-H/2,H/2]$.
...
The \textbf{cost} of a naive tensor discretization with $N_z\sim N$ nodes in~$z$ and $N^2$ nodes in $(T,q)$ per vertical layer is $\mathcal{O}(N^3)$ evaluations of~$g$ (and any functions composed with it).
The \profileros\ reduction achieves $\mathcal{O}(N^2)$ under the stated linear-mean and homogeneous-covariance assumptions.

\paragraph{Two-half face model (single oracle).}
For one column cell, calibration utilities build \textbf{one} mixture marginal $p_{\mathrm{mix}}(T,Q)$ in $(T,q)$ by averaging, for each $z_k$ on $[0,L_{\!1/2}]$, the sum of DN- and UP-facing Gaussians with \emph{distinct} mean slopes toward each face and face-anchored linear $\Sigma(z)$ (including optional linear drift of $\mathrm{cov}_{Tq}$ along each half). The condensate expectation $\mathbb{E}[\max(q-q_{\mathrm{sat}}(T),0)]$ under this law is the \textbf{numerical oracle} (dense $(T,q)$ grid and/or tensor Gauss--Hermite in $(T,q)$ at each~$z$, averaged in~$z$). Any other discretization advertised for the \textbf{same} cell---including the two-half \profileros\ cell construction used in production---must converge to that oracle as resolution increases; a persistent gap indicates \textbf{wrong or incomplete code}, not a second acceptable answer. Implementations use stable $\mathrm{erfc}/\mathrm{erfcx}$ segment kernels and the $\nu\in\{3/2,1/2,-1/2\}$ reduction for linear-in-$z$ variance laws; see \texttt{profile\_rosenblatt\_two\_half\_cell.py}, \texttt{ORACLE\_SGS.md}, and high-precision tests.
...
\subsection{Mismatched rules: why Hermite nodes are wrong for the inner marginal}
Along the inner axis of the \profileros\ scheme (at fixed outer~$v$), the relevant 1D law is the \textbf{bounded Gaussian--uniform convolution}: convolution of a Gaussian with a uniform on $[-L/2,L/2]$.
...
This is the source of all ``$p_i=(t_i+1)/2$'' factors in the inverse-CDF inner rule.
...
The valid 3D structure is \textbf{mixed}: compact uniform~$z$ (after normalization) $\times$ non-compact Gaussian in~$\mathbf{x}$ at each~$z$ (\columntensor), or, after \profileros, uniform-in-$z$ compressed into a bounded inner marginal times a Gaussian in~$v$.
...
Rosenblatt's transform~\cite{rosenblatt} systematizes monotone triangular maps that factor a multivariate law into univariate conditionals; the \profileros\ construction is the instance aligned with the mean gradient~$\mathbf{d}$ in which the inner marginal becomes the Gaussian--uniform convolution described in \S\ref{sec:pill-marginal}.
...
Under linear mean in~$z$ and homogeneous $\Sigma_{\mathrm{turb}}$, the $z$-average and bivariate Gaussian can be combined so that $\overline{q_c}$ becomes a double sum: an outer quadrature in a Gaussian coordinate~$v$ transverse to~$\mathbf{d}$ in state space, and an inner quadrature in~$u$ along~$\mathbf{d}$ against the \textbf{bounded inner} marginal at each~$v$ (Sections~\ref{sec:grad-align-coords}--\ref{sec:profros-assembly}).
...
%==============================================================================
\section{\texorpdfstring{The bounded inner marginal along $u$ at fixed $v$}{Bounded inner marginal along u at fixed v}}
...
Their sum is the \textbf{bounded Gaussian--uniform convolution}; its density is trapezoidal in the center with Gaussian tails.
...
Nodes $u_i=F^{-1}(p_i)$ are \textbf{quantiles} of this marginal; this is standard \emph{inverse-CDF} or \emph{quantile} quadrature~\cite{gautschi}.
...
Here $F$ is explicit~\eqref{eq:pill-cdf} but $F^{-1}$ is not elementary.
...
\begin{algorithm}[H]
\caption{Brent quantiles for bounded inner marginal}
...
For each outer node~$v_j$, \profileros\ needs $\{u_i\}$ at $s=s_{u\mid v}(v_j)$: $\mathcal{O}(N)$ Brent solves along the conditional curve, $\mathcal{O}(N^2)$ per layer if done naively.
...
%==============================================================================
\section{Surrogate quantile inversions}
...
Online: evaluate degree-$D$ Chebyshev sums---$\mathcal{O}(ND)$ flops per layer versus $\mathcal{O}(N)$ iterative solves $\times$ cost per iteration for Brent.
...
Define (subscript $\mathrm{PR}$ denotes \profileros; $\mathrm{B}$ / $\mathrm{R}$ denote Brent vs.\ rational inversion)
\begin{align}
...
$e_{\mathrm{pill}}$ \textbf{isolates} inverse-CDF surrogate error with all other \profileros\ choices identical (subscript kept for compatibility with earlier drafts).
...
%==============================================================================
\section{\texorpdfstring{Worked micro-example ($N=2$ inverse-CDF quantiles)}{Worked micro-example N equals 2 inverse-CDF quantiles}}
...
One atmosphere-model codebase exposes helpers such as \texttt{gauss\_hermite}, \texttt{gauss\_legendre\_01}, and gradient-aware SGS drivers; the full \profileros\ pipeline (gradient-aligned map~$M$, bounded-marginal quantiles, Brent reference, cached surrogates) may not be complete in any given repository version.
