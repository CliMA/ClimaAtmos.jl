...
|--|--|--|
| **EKI** | **One** calibration with **`sgs_quadrature_subcell_geometric_variance: false`** (varfix **off**). | **Two** calibrations: one YAML with varfix **off**, one with varfix **on** (distinct `output_dir` / `observations_path` per slice). Listed in [`va_calibration_sweep_configs()`](lib/calibration_sweep_configs.jl). |
...
1. **Calibrated, varfix off:** `sgs_quadrature_subcell_geometric_variance: false`, run **`scripts/run_calibration.jl`** (or include that YAML in [`va_calibration_sweep_configs()`](lib/calibration_sweep_configs.jl) and run [`scripts/run_calibration_sweep.jl`](scripts/run_calibration_sweep.jl)).
