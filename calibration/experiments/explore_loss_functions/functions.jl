...
function H(dir::String, short_name, iteration, member)
    simdir = ClimaCalibrate.path_to_ensemble_member(dir, iteration, member)

    H_perf(simdir, short_name, cutoff=1, output_cov=false)
end
...
function calibrate(n_iters, ens_size, obs_mean, obs_cov, prior, scheduler, job_id, short_name)
    ...
    for iteration in 0:n_iters
        ...
        for m in 1:ens_size # parallel loop of ensemble members 
            ...
            try
                ...
                G_ensemble[:, m] .= H("output/$job_id", short_name, iteration, m)
            catch err
