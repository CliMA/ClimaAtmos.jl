...
function generate_atmos_configs(base_config_path::String, parameter_path::String, run_output_dir::String)
    ...
    atmos_configs = map(collect(1:num_cases)) do i
        ...
        if haskey(experiment_config_dict, "forcing_toml_files") && haskey(experiment_config_dict["forcing_toml_files"], forcing_type)
            forcing_config_file = experiment_config_dict["forcing_toml_files"][forcing_type]
            # Assume forcing configs are in a 'configs' subdir relative to the main calibration output dir
            forcing_config_path = joinpath(calib_output_dir, "configs", forcing_config_file)
