...
function valid_lespath(les_path)
    ...
    @assert cfsite_number in
            keys(LES_library[forcing_model][month]["cfsite_numbers"]) "cfSite $(cfsite_number) not found for $(forcing_model), month $(month)."
...
function get_shallow_LES_library()
    ...
    LES_library["HadGEM2-A"]["07"]["cfsite_numbers"] =
        deepcopy(Shen_et_al_sites)
    LES_library["HadGEM2-A"]["04"] = Dict()
    LES_library["HadGEM2-A"]["04"]["cfsite_numbers"] =
        setdiff(Shen_et_al_sites, [15, 17, 18])
    ...
    LES_library["CNRM-CM5"]["10"]["cfsite_numbers"] =
        setdiff(Shen_et_al_sites, [15, 22, 23])
    LES_library["CNRM-CM5"]["07"] = Dict()
    LES_library["CNRM-CM5"]["07"]["cfsite_numbers"] =
        setdiff(Shen_et_al_sites, [13, 14, 15, 18])
    ...
    LES_library["CNRM-CM5"]["01"]["cfsite_numbers"] =
        setdiff(Shen_et_al_sites, [14, 15, 17, 18, 19, 20, 21, 22, 23])

    # CNRM-CM6-1 model (69 AMIP-AMIP4K pairs)
    LES_library["CNRM-CM6-1"]["10"] = Dict()
    LES_library["CNRM-CM6-1"]["10"]["cfsite_numbers"] =
        setdiff(Shen_et_al_sites, [22, 23])
    LES_library["CNRM-CM6-1"]["07"] = Dict()
    LES_library["CNRM-CM6-1"]["07"]["cfsite_numbers"] =
        setdiff(Shen_et_al_sites, [12, 13, 14, 15, 17])
    LES_library["CNRM-CM6-1"]["04"] = Dict()
    LES_library["CNRM-CM6-1"]["04"]["cfsite_numbers"] =
        setdiff(Shen_et_al_sites, [13, 14, 15])
    LES_library["CNRM-CM6-1"]["01"] = Dict()
    LES_library["CNRM-CM6-1"]["01"]["cfsite_numbers"] =
        setdiff(Shen_et_al_sites, [14, 15, 21, 22, 23])
