...
function get_obs(
    ...
    #         push!(Σ_list, Σ_const[y_name] * ones(length(z_scm)))
    #     end
    # elseif Σ_scaling == "prop"
    #     for y_name in y_names
    #         Σ_list = [Σ_const[y_name] * norm_vec[i, 2] for i in 1:length(y_names)]
    #     end
    # else
    #     throw(ArgumentError("Σ_scaling must be either 'const' or 'prop'."))
    # end

    # Σ =  Diagonal(vcat(Σ_list...))
