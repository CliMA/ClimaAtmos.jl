...
const config_dict = YAML.load_file(joinpath(output_dir, "configs", "experiment_config.yml"))
...
function inv_variance_weighted_loss(y_diff::Matrix{Float64}, gamma_i::Matrix{Float64})
...
for iteration in iterations
    ...
    cov_init = cov(u, dims = 2)
     ...
     avg_var_loss, std_var_loss, var_min_loss = inv_variance_weighted_loss(y_diff, gamma_i)
    ...
    plot!(plt, x_range, y_i .+ 2 * noise_std, fillrange = y_i .- 2 * noise_std, fillalpha = 0.2, label = "Noise range", color = :gray)
...
function plot_loss(iter_range, avg_loss, std_loss, min_loss, title_str, file_name)
    plt = plot(iter_range, avg_loss, label = "$(title_str) Avg", lw=2, xlabel = "Iteration", ylabel = "Loss", title = title_str, marker=:o, color=:black)
    upper_2std = avg_loss .+ 2 .* std_loss
    lower_2std = avg_loss .- 2 .* std_loss
    plot!(plt, iter_range, upper_2std, fillrange=lower_2std, label="±2 STD Shading", fillalpha=0.3, lw=0, color=:blue)
    plot!(plt, iter_range, min_loss, linestyle=:dash, color=:black, label="Min $(title_str)")
    savefig(plt, joinpath(plot_dir_y_vec, file_name))
end

iterations_range = 1:length(iterations)

plot_loss(iterations_range, loss_results[:mean_loss_avg], loss_results[:mean_loss_std], loss_results[:min_loss_avg], "Mean Loss", "mean_loss_vs_iteration.png")
plot_loss(iterations_range, loss_results[:var_weighted_loss_avg], loss_results[:var_weighted_loss_std], loss_results[:var_weighted_min_loss_avg], "Variance-Weighted Loss", "var_weighted_loss_vs_iteration.png")
...
for param_i = 1:size(phi_all_stacked, 1)
    ...
    plot!(iterations, max_vals, label="Max", lw=1, linestyle=:dash, color=:black)
    plot!(iterations, min_vals, label="Min", lw=1, linestyle=:dash, color=:black)

    plot!(iterations, upper_2std, fillrange=lower_2std, label="±2 STD Shading", fillalpha=0.3, lw=0, color=:blue)
