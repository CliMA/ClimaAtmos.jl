...
function CAL.forward_model(iteration, member, config_dict = nothing)
    ...
    if isnothing(config_dict)
        config_dict =
            YAML.load_file(joinpath(experiment_dir, "diagnostic_edmfx_diurnal_scm_imp_noneq_1M.yml"))
    end
    output_dir = "/home/oalcabes/EKI_output/test_4" #config_dict["output_dir"] # need to input
