...
function H_perf(dir::String, short_name; noise = 0.01, cutoff=50, output_cov = true)
    ...
    if output_cov
        ...
        obs_cov = noise .* Matrix(I, length(obs_mean), length(obs_mean)) #cov(obs_dat)
        #obs_cov = cov(obs_dat)+ .01 * maximum(cov(obs_dat)) * I
