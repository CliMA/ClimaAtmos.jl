...
output_dir = "/central/groups/esm/jschmitt/calibrations/diurnal_diag_3"
iterations = 0:9
# iterations = nothing
...
for iteration in iterations
    ...
    for i in 1:size(g_non_nan, 2)
        Plots.plot!(plt, 1:size(g_non_nan, 1), g_non_nan[:, i], label = false)
    end
    Plots.plot!(plt, 1:size(y_i, 1), y_i, label = "Observations", color = :black)
