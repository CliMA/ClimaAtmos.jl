...
function plot_jacobian(output_path, time_series, file_name, Yₜ_end, dt)
    ...
    abs_Yₜs =
        map(Yₜ -> (@. ifelse(iszero(Yₜ) || issubnormal(Yₜ), NaN, abs(Yₜ))), Yₜs)
    min_abs_Yₜ = minimum(abs_Yₜ -> minimum(filter(!isnan, abs_Yₜ)), abs_Yₜs)
    max_abs_Yₜ = maximum(abs_Yₜ -> maximum(filter(!isnan, abs_Yₜ)), abs_Yₜs)
    ...
    for (page_index, (is_rescaled, transform, transform_string)) in enumerate((
        ...
        max_logabs = maximum(matrices) do matrix
            maximum(filter(!isnan, matrix); init = floatmin(eltype(matrix)))
        end
        figure = CairoMakie.Figure(; figure_kwargs...)
    ...
    for (t_index, (t, title, abs_Yₜ)) in enumerate(zip(times, titles, abs_Yₜs))
        grid_position = figure[cld(t_index, 2), (t_index - 1) % 2 + 1]
        Yₜ_title = replace(title, r".+∂Yₜ/∂Y" => "Yₜ")
        axis =
