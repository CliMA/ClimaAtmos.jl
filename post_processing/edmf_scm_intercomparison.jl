...
#   Row 2 profiles : a_up [1:2] | w_up [3:4] | TKE [5:6] | CF [7:8] | u & v [9:12]
#   Row 3 timeseries: LWP [1:3] | RWP [4:6] | IWP [7:9] | SWP [10:12]
#
# Usage:
#   julia --project=.buildkite post_processing/edmf_scm_intercomparison.jl \
...
# "pycles_scm_les_data" (populated from the root Artifacts.toml) and
...
# ── Figure ─────────────────────────────────────────────────────────────────────
# 12-column grid: Row 1 profile panels (×6) each span 2 cols; Row 2 has 5
# panels (first 4 × 2 cols, u/v panel spans remaining 4 cols); Row 3
# timeseries panels (×4) each span 3 cols.

const REF_COLOR = "#e07b39"   # orange – PyCLES
const V_REF_COLOR = "#c0392b"   # red    – PyCLES v
const SIM_COLOR = "#2e6da4"   # blue   – ClimaAtmos
...
# Row 2: dynamics + turbulence (first 4 panels × 2 cols; u/v panel × 4 cols)
...
ax12 = make_profile_ax(fig[2, 9:12], "u, v (m/s)", "Horiz. translational velocity")
...
if has_sim
    ...
    # u/v: both models now in the absolute (Earth) frame — no offset needed.
    # PyCLES uses u_translational_mean / v_translational_mean; ClimaAtmos ua / va.
    _z_u, _mu_u, _sig_u = load_and_avg("ua")
    if !isnothing(_z_u)
        profile_band!(ax12, _mu_u, _sig_u, _z_u ./ 1e3, SIM_COLOR)
        sim_line!(ax12, _mu_u, _z_u ./ 1e3; label = "u  ClimaAtmos")
    end
    _z_v, _mu_v, _sig_v = load_and_avg("va")
    if !isnothing(_z_v)
        profile_band!(ax12, _mu_v, _sig_v, _z_v ./ 1e3, V_SIM_COLOR)
        sim_line!(ax12, _mu_v, _z_v ./ 1e3; label = "v  ClimaAtmos", color = V_SIM_COLOR)
    end
...
# meaningful than PyCLES zeros (warm cases, missing vars, etc.)
if has_sim
    for (ax, sn, scale) in [
        (ax4, "husra", 1e3),
        (ax5, "cli", 1e3),
...
# u/v axis: union of PyCLES and ClimaAtmos ranges (both in absolute frame).
let uv_data = Tuple{Vector{Float64}, Vector{Float64}}[]
    ...
    if has_sim
        ...
        !isnothing(mu_u) && push!(uv_data, (mu_u, sig_u))
        !isnothing(mu_v) && push!(uv_data, (mu_v, sig_v))
    end
    if !isempty(uv_data)
        ...
        CairoMakie.xlims!(ax12, all_lo - m, all_hi + m)
    end
end
