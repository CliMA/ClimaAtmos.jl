...
function make_plots(::Larcform1Plots, output_paths::Vector{<:AbstractString})
    ...
    short_names_2D = filter(sn -> sn in available_short_names, short_names_profile_requested)
    short_names_1D =
    ...
    function seconds_to_hours(var)
        ...
        if !haskey(var.dim_attributes, t_name) || !haskey(var.dim_attributes[t_name], "units")
    ...
    surface_names = ["ts", "hfss", "hfls", "rlds", "rlus", "rlut", "rlutcs", "pr", "prsn", "evspsbl"]
