...
const PeriodicTopographyTest2D = Union{
    ...
    Val{:gpu_plane_no_topography_long_float64_test},
...
function make_plots(val::TopographyTest, output_paths::Vector{<:AbstractString})
    ...
    for velocity_short_name in ("ua", "wa")
        GC.gc(true) # may help avoid out-of-memory errors for 3D vars
        short_names = velocity_short_name .* ("error", "", "predicted")
        make_plots_generic(
            output_paths,
            [Iterators.flatmap(slice_summary_vars, short_names)...];
            output_name = "slice_summary_" * velocity_short_name,
            plot_fn = plot_contours!,
            rasterize = true, # may help avoid out-of-memory errors for 3D vars
        )
    end
end
