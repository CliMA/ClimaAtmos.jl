...
## Codebase map
  ...
  - `src/parameterized_tendencies/`: parameterization implementations.
      + `aerosols/`: aerosol species dispatch (`aerosols.jl`) and prognostic sea salt: emission, spectrum moments and tendency assembly (`sea_salt.jl`), hygroscopic growth (`hygroscopic_growth.jl`), gravitational settling (`settling.jl`), turbulent dry deposition (`dry_deposition.jl`), species-agnostic lognormal spectrum moments (`lognormal_moments.jl`).
