...
!!! note
    ...
    The strong- and weak-form horizontal spectral operators are unified in
    ClimaCore: `Divergence`, `Gradient`, and `Curl` take a form-type parameter
    (`StrongForm`, the default, or `WeakForm`), so the weak divergence, for
    example, is `Divergence{WeakForm}`. The legacy aliases (`WeakDivergence`,
    `WeakGradient`, `WeakCurl`) were removed in ClimaCore v1.
...
horizontal discretization would replace this list.
  ...
  - ``\hat{\mathcal{D}}_h`` is the weak horizontal spectral divergence, the
    `WeakForm` variant of [`ClimaCore.Operators.Divergence`](@extref).
  ...
  - ``\hat{\mathcal{G}}_h`` is the weak horizontal spectral gradient, the
    `WeakForm` variant of [`ClimaCore.Operators.Gradient`](@extref); it is the
  ...
  - ``\hat{\mathcal{C}}_h`` is the corresponding weak curl, the `WeakForm`
    variant of [`ClimaCore.Operators.Curl`](@extref).
