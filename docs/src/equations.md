...
The vorticity hyperviscosity coefficient scales with the mean nodal distance ``h`` as ``\nu_4 = c_4 \, h^3`` ([Lauritzen et al. 2018](https://doi.org/10.1029/2017MS001257)), where ``c_4`` is `vorticity_hyperdiffusion_coefficient`.
...
The coefficient factor ``F = \max(\text{divergence damping factor}, 1/\mathrm{Pr})`` is the largest of the divergent-momentum (``\times`` divergence damping factor), scalar (``\times 1/\mathrm{Pr}``), and rotational (``\times 1``) coefficient factors.
