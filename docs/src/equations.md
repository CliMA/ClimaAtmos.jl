...
lets one equation set cover Cartesian geometries, for cloud-resolving
simulations, and the sphere, for global weather and climate simulations.
...
stratosphere. Two of the three numbers in it are configurable:
  ...
  - The temperatures ``T_{sfc}`` and ``T_{\min}`` are Thermodynamics parameters, defaulting to
  ...
  - The exponent ``n_s`` is `s_ref`, fixed at 7 by a constant in
