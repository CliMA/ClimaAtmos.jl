...
The grid factor ``\beta`` is defined by ``\rho(\nabla^4) = (\beta/h)^4``, the largest eigenvalue of the discrete horizontal biharmonic.
The limit uses the upper bound ``\beta \le \beta_\mathrm{op}(p) \, M``, the uniform-grid factor composed with the peak metric factor; the bound is exact on a uniform grid (``M = 1``) and conservative on an anisotropic grid, where it overestimates the assembled operator's spectral radius.
The uniform-grid factor ``\beta_\mathrm{op}(p)`` is obtained from the spectral radius of the assembled scalar operator ``(\hat{\mathcal{D}}_h \circ \mathcal{G}_h)^2`` on a degree-``p`` element via ``\rho(\nabla^4) = (\beta_\mathrm{op}/h)^4`` (generated by `test/prognostic_equations/hyperdiffusion_grid_factor.jl`):
...
A direct spectral-radius measurement of the assembled cubed-sphere biharmonic (``h_\mathrm{elem} = 6``, degree 3) gives ``\beta \approx 5.19`` under the same length scale, which the factorized bound ``\beta_\mathrm{op} \, M \approx 6.14`` overestimates by about 18%, so the resulting limit is conservative and inside the measured bracket.
...
The formula reproduces two measured stability limits with no tuned constant, at ``c_4 = 0.1857``, ``\mathrm{Pr} = 0.2``, and divergence damping factor ``5``: the forward-Euler limit is ``\approx 0.9`` s at ``h = 113`` m on a degree-3 dry density-current box (measured stable at ``0.9`` s, unstable at ``1.2`` s), and ``\approx 780`` s on the dry baroclinic-wave cubed sphere at ``h_\mathrm{elem} = 6`` (measured bracket ``[720, 1600]`` s).
...
For polynomial degrees outside the tabulated range the limit is likewise unavailable: no warning is emitted, and a positive `hyperdiffusion_dt_safety_factor` raises an error at construction that names the generating script.
