...
```
- **Heating depth** $h = z_{\text{top}} - z_{\text{bot}}$ — the vertical extent of the convective envelope. It is set by *moment-matching* a half-sine to the in-cloud heating $Q_{\text{ic}}$ through its first two vertical moments — the heating centroid $z_c$ and spread $\sigma$ (both taken over $z \geq z_{\text{bot,floor}}$, `nogw_beres_z_bot_floor`, to reject the boundary-layer / dry-thermal signal). A half-sine of depth $h$ has variance $h^2(\pi^2-8)/(4\pi^2)$, so $h = \sigma / \sqrt{(\pi^2-8)/(4\pi^2)}$, and $z_{\text{bot}}, z_{\text{top}} = z_c \mp h/2$ (with $z_{\text{top}}$ clamped to the domain top and $z_{\text{bot}} \geq 0$). This seats the envelope on the actual heating peak. (An earlier *area-threshold* construction — $z_{\text{top}}$ from the updraft-area extent and $z_{\text{bot}}$ from a grid-mean $Q_1$ threshold above the floor — is deprecated and no longer reachable from config; it was often narrow and misplaced relative to the in-cloud heating peak.)
