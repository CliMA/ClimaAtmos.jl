...
```
- **Heating depth** $h = z_{\text{top}} - z_{\text{bot}}$ — the vertical extent of the convective envelope: $z_{\text{top}}$ is the highest level where the total updraft area fraction exceeds $10^{-3}$ (the plume top), and $z_{\text{bot}}$ is the lowest level that lies above an altitude floor $z_{\text{bot,floor}}$ (`beres_z_bot_floor`) **and** carries convective heating above a threshold, $Q_1 > Q_{\text{bot,thresh}}$ (`beres_z_bot_Q_threshold`). The altitude floor is essential because EDMF $Q_1$ has a strong boundary-layer / dry-thermal signal near the surface that would otherwise pull $z_{\text{bot}}$ down to the ground; the heating threshold marks the base of deep convective heating. (See `default_config.yml` for current values.) This *area-threshold* construction is the default (`beres_envelope_mode = area_threshold`); an alternative `moment_matched` mode instead fits the half-sine to the in-cloud heating $Q_{\text{ic}}$ through its first two vertical moments — centroid $z_c$ and spread $\sigma$ — setting $h$ from the half-sine variance relation and $z_{\text{bot}}, z_{\text{top}} = z_c \mp h/2$, which seats the envelope on the heating peak rather than on the updraft-area extent.
...
The transient spectrum above starts the frequency integral at $\nu_{\min} > 0$ and therefore omits the steady ($\nu = 0$) part of the heating. When `beres_steady_source` is enabled (off by default), this steady part is added as a single ground-stationary wave: the time-mean convective heating acts like a fixed obstacle in the mean wind $\bar{u}_{\text{heat}}$ and radiates one mountain-wave-like mode with vertical wavenumber $m_0 = N / |\bar{u}_{\text{heat}}|$ (Beres Eqs. 32–33), signed to oppose $\bar{u}_{\text{heat}}$. It is deposited entirely into the $c \approx 0$ phase-speed bin — which the transient spectrum leaves empty, so there is no double-counting; construction requires an exact $c = 0$ grid point (`cmax/dc` an integer). The steady amplitude carries the same half-sine response factor and the same scale factor $\alpha$ as the transient spectrum, so the two are not independently tunable; its only additional inputs are a zero-frequency temporal weight $|Q_t(0)|^2$ set by a DC-weight knob (`beres_steady_dc_frac`) and a largest convective-system scale (`beres_L_system`) that fixes the horizontal projection. Whether to include it is left optional because, per Beres' §5, the relative importance of the steady and transient components depends on the convective regime.
