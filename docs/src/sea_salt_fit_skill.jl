...
function stored_params()
    ...
    return modes, r_hat_edges, params.ssa_r80_per_dry
end

"""
    skill(modes, r_hat_edges, r80_per_dry)

Deviations of the lognormal `modes` from the exact Gong spectrum in dry-radius
coordinates: max pointwise relative error over r̂ ∈ [0.015, 10], and per-bin
relative errors of the number (0th-moment) and dry-mass (3rd-moment) fluxes
over the `r_hat_edges` bins. Constant moment prefactors cancel in the ratios.
"""
function skill(modes, r_hat_edges, r80_per_dry)
