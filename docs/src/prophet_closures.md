...
| ``\mathrm{tke}_0``                                                  | `sgs_variance_geometric_tke_scale` (TKE weight ``w = \mathrm{tke}_0/(\mathrm{tke}_0 + \max(\mathrm{tke},0))``; 0 = off)                | — (provisional)                                                                                         |
| ``\theta_{z,\min}``                                                 | `sgs_variance_isentropic_min_dtheta_dz` (regularisation of ``\partial\theta_{li}/\partial z`` in the isentropic slope ratio)           | — (provisional)                                                                                         |
| ``r_{\max}^{\mathrm{isen}}``                                        | `sgs_variance_isentropic_slope_cap` (cap on the isentropic slope ratio ``(\partial q_t/\partial z)/(\partial\theta_{li}/\partial z)``) | — (provisional)                                                                                         |
| ``r_{\mathrm{corr,max}}``                                           | `sgs_correlation_max` (clamp on the diagnosed ``T``–``q_t`` correlation; `tq_correlation_model: diagnosed`)                            | — (provisional)                                                                                         |
