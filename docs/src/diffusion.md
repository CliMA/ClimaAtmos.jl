...
configuration key. Both are boundary-layer schemes rather than resolved-eddy
closures: the diffusivity is a function of height or pressure, not of the
resolved strain rate.

| `vert_diff`                  | Type                                          | Eddy diffusivity                                                                 |
|:---------------------------- |:--------------------------------------------- |:-------------------------------------------------------------------------------- |
| `~` (default)                | none                                          | No vertical diffusion                                                            |
| `"VerticalDiffusion"`        | [`ClimaAtmos.VerticalDiffusion`](@ref)        | ``K_E = C_E \|\boldsymbol{u}_a\| z_a`` below 850 hPa, decaying above (see below) |
...
lowest model level, and ``C_E`` is a dimensionless coefficient. Above 850 hPa
the surface-driven diffusivity decays as
...
acting [Smagorinsky–Lilly](les_sgs.md) closure. Those three paths are
alternatives, not additions: `vert_diff` is left unset in the PROPHET and LES
configurations.
...
microphysics rather than through turbulent mixing; see
[Microphysics](microphysics.md).
...
to all grid-scale tracers and exempt nothing, while anisotropic minimum
