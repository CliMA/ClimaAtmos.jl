...
Hyperdiffusion is numerical rather than physical, which separates it from the
[diffusion](diffusion.md) that represents turbulent transport: it stabilizes the
...
where ``q_v^*`` is the saturation specific humidity over liquid, evaluated at
the dry reference density ``\rho_r = p / (R_d T_r)``, and ``q_{t,r}`` is set to
zero above 250 hPa, where the reference profile is no longer a useful
approximation to the moisture field.

The same reference geopotential ``\Phi_r`` appears in the pressure-gradient
force; see [Governing Equations](equations.md).

The tendency is applied as two successive Laplacians with the inter-element
completion step applied in between (direct stiffness summation on the
continuous elements ClimaAtmos uses; see
[Completing a tendency across elements](@ref)). This removes the
discontinuities across element boundaries and preserves the discrete inner
product, so the operation remains globally conservative. In the code, this is the split between
...
to offset them. Here, each hyperdiffusive term is a flux divergence along a
coordinate surface, so the operators redistribute quantities and conserve the
global total.
...
velocity,
``\boldsymbol{\mathcal{H}}_u = \nu_u \nabla_h (\Delta_h \boldsymbol{u})``,
with the horizontal vector Laplacian
...
the model uses the simpler form common in atmosphere models,
...
density variations along horizontal coordinate surfaces. Values
``\delta_{div} > 1`` increase the damping of two-dimensional divergent flow
components, which include gravity waves; the default is ``\delta_{div} = 5``,
set by the
...
| Coefficient defaults        | [config/default_configs/default_config.yml](https://github.com/CliMA/ClimaAtmos.jl/blob/main/config/default_configs/default_config.yml)   |
...
[Configuration Options](configuration_options.md). All three coefficients are
YAML keys read from the configuration, not ClimaParams entries, so a TOML
