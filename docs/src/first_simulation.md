...
`hours`, `days`, `weeks`), as does the timestep `dt`. Other aspects of the
simulation have keyword arguments too; omitted ones take their defaults.
...
The default global simulation is slow to integrate on a laptop. Presets bundle
a grid, a setup, and matching physics into a single call, which is the quickest
route to a running case — here a fast-running column with the BOMEX
shallow-cumulus initial state and moist physics. `solve_atmos!` advances it to
`t_end`, and the integrator time confirms where the run stopped:
...
Presets do more than save typing: the `setup` argument sets the initial state,
while the physics comes from the model, so the two have to be chosen together.
BOMEX with the default dry model would have no moisture. Each preset pairs a
setup with a matching grid and model. The pairing is minimal rather than
complete: `Presets.bomex` enables moist physics but no
