...
small tracer damping. No grid-mean tracer receives Rayleigh damping: the
coefficient acts on the turbulence kinetic energy and on the PROPHET subdomain
scalars, and for the latter the relaxation is toward the grid-mean value rather
than toward zero, so only the subgrid-scale departure is damped.
