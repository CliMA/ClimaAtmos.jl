...
Unlike the prescribed vertical diffusion described below, no species is exempt
and no redistribution among cloud species takes place.
...
Unlike Smagorinsky–Lilly, the closure has no axis switch. Setting
`amd_les: true` applies both the horizontal and the vertical tendency.
...
a prescribed diffusivity profile, not a resolved-eddy closure, and it differs in
