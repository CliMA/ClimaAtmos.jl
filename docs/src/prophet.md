...
The equations hold whether or not convection is resolved. As the
...
*environment*. It is an ordinary subdomain, distinguished only by having its prognostic equations eliminated. The remaining subdomains
...
are carried separately outside the buoyancy terms: a fully compressible model needs no anelastic approximation within the subgrid, and one would complicate exact energy consistency.
