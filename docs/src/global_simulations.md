...
layer, so that the boundary layer is resolved without uniform spacing aloft.

Rather than assembling these keys yourself, start from one of the numerical
configurations in `config/common_configs/`, which pair a resolution with a
timestep and a matching set of numerics:
...
Two related keys matter at these resolutions:
  ...
  - `dt_rad` sets how often radiation is called, 6 hours by default and 1 hour
    ...
    other parameterization in a global run, so this interval is worth setting
    deliberately rather than inheriting; see
    [Running with Radiation](radiation_howto.md).

An explicit parameterization with its own stability limit can also bind before
the dynamics do. Explicit horizontal diffusion is the usual example; see
...
it prescribes sea surface temperatures rather than computing them. Coupled
