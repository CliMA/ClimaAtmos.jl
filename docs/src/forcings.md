...
!!! note "TODO: rain and snow subside only through total water"
...
one term and the two wind components as another. The separation matters:
`VerticalFluctuation` supplies a fixed tendency computed from the driving data,
whereas `Subsidence` transports whatever the column currently holds. A dataset
that already contains the full vertical advective tendency should use one or the
other, not both.
