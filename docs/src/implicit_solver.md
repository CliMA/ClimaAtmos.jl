...
The tendency is evaluated in two calls: ``p_{imp}(Y)`` computes
...
This algorithm is a reference rather than a production choice: it treats the
matrix as dense, so it is the standard against which the two sparse algorithms
are checked.
