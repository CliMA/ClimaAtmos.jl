...
This is a SMOKE test: short `t_end` (30 mins). To run the longer (6 h / 12 h) validation.
The post-solve assertions (crash / callback / reached-t_end / finite-state) are the
CI pass/fail gate — any crash, NaN, or early exit throws and fails the job.
"""
...
model = CA.AtmosModel(;
    ...
    # NOTE: the current API has no `surface_model`/`PrescribedSST` and no
...
## Grid creation
# NOTE: the YAML path's `config: column` does NOT use a 1D `ColumnGrid` — it uses
