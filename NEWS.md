...
- [#4653](https://github.com/CliMA/ClimaAtmos.jl/pull/4653) ![][badge-🔥behavioralΔ] Improve the manual implicit Jacobian (the tendencies are unchanged; behavior changes only through the single-Newton-iteration implicit solve).
  - Document the exactness of the pressure-gradient-force Jacobian (stencils unchanged): the complete linearization of the Exner-form tendency `cp_d θ_v ∇Π` — thermal buoyancy `cp_d ∂θ_v ∇Π`, acoustic `∇δp/ρ`, and the pressure-buoyancy commutator `cp_d θ_v ∇(κ_d Π/p) δp` — collapses via the equation-of-state identity `δρ/ρ = (1-κ_d) δp/p - δθ_v/θ_v` to the acoustic operator in every column perturbed at fixed density plus a single buoyancy term in the `ρ` column, which is the existing height-form Jacobian. Thermal and pressure buoyancy cancel analytically rather than numerically, and sound and gravity waves are both treated fully implicitly at no added cost.
