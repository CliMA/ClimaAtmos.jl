...
- [#4668](https://github.com/CliMA/ClimaAtmos.jl/pull/4668) ![][badge-✨feature/enhancement] Scale the vorticity hyperdiffusion coefficient with the acoustic-substepping outer step (`acoustic_substep_hyperdiffusion_scaling`). Under substepping the frozen hyperdiffusion filter is integrated at the outer step, so its explicit stability limit bounds the outer-step multiple; `auto` (the default) reduces the coefficient to keep the filter within that limit, lifting the ceiling in proportion to `1/ν₄`. A number scales the coefficient directly, with `1` reproducing the unscaled coefficient. The scaling only affects substepping runs (`acoustic_substeps` nonzero); the default configuration and hyperdiffusion-off runs are unchanged. See `docs/src/acoustic_substepping.md`.
