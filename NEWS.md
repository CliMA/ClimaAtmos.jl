...
- [#4647](https://github.com/CliMA/ClimaAtmos.jl/pull/4647) ![][badge-🔥behavioralΔ] Make the EDMFX (PROPHET) boundary-layer representation less sensitive to vertical resolution by reconstructing subgrid inversion structure.
  ...
  - Make the stability closure interface-aware: an effective `N²_eff = N² + [(Δb)₊]²/(c_b·κ_iso)` senses unresolved buoyancy jumps across faces, and a gated interfacial entrainment diffusivity `K_e = γ·w_e·Δz` (with `w_e = A√κ/max(Ri_b, 1)`) restores the physical, finite entrainment across the reconstructed inversion; `A` is the new calibratable parameter `EDMF_interface_entr_efficiency` (default 0, closure off).
  - Evaluate the stability closure and eddy diffusivities natively at cell faces (`ᶠK_h`, `ᶠK_u`, `ᶠK_entr`, exact two-point differences) and cache the master mixing length (`ᶜl_mix`); factor the buoyancy-gradient thermodynamics into cloud-fraction-independent coefficients. The TKE budget uses the same face diffusivities as the fluxes.
