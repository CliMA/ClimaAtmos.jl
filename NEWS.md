...
- [#4704](https://github.com/CliMA/ClimaAtmos.jl/pull/4704) ![][badge-🔥behavioralΔ] Diffuse total enthalpy in the Smagorinsky-Lilly and AMD tendencies, vertical and horizontal, with the single-gradient decomposition `∇s_d + (h_eff + Φ) ∇q_tot_eff` used by the boundary-layer and EDMFX diffusion, instead of diffusing `h_tot` directly. The decomposition removes the spurious enthalpy flux produced by the dry-air-mass gradient and the diffusive flux of kinetic energy. AMD keeps evaluating its energy diffusivities from the total-enthalpy gradient.
