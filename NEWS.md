...
- [#4647](https://github.com/CliMA/ClimaAtmos.jl/pull/4647) ![][badge-🔥behavioralΔ] Make the EDMFX (PROPHET) boundary-layer representation less sensitive to vertical resolution by reconstructing subgrid inversion structure.
  ...
  - Evaluate the stability closure and eddy diffusivities natively at cell faces (`ᶠK_h`, `ᶠK_u`, `ᶠK_entr`, exact two-point differences) and cache the master mixing length (`ᶜl_mix`); factor the buoyancy-gradient thermodynamics into cloud-fraction-independent coefficients. The TKE budget uses the same face diffusivities as the fluxes. The `edt`/`evu` diagnostics now report these effective coefficients (including the interfacial entrainment contribution) instead of recomputing a center-based diffusivity, and the interfacial entrainment diffusivity is output as the new default EDMFX diagnostic `kentr`.
