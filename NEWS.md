...
main
----
- ![][badge-🐛bugfix] Split the `dynamics` CI group in two. Its ten files took 75 to 92 minutes on GitHub's runners against the job's 90-minute limit, and a job cancelled at the limit reports as failed while never running the files that had not started, so every pull request's `ci 1.10 - dynamics` job was a coin toss and a rerun. The two EDMFX diffusion files, which build a fresh EDMFX model per configuration and recompile the tendency pipeline each time, now run as `dynamics_edmfx`; the other eight stay in `dynamics`. Both workflow matrices and the test-group table list the new group.
