...
- [#XXXX](https://github.com/CliMA/ClimaAtmos.jl/pull/XXXX) ![][badge-✨feature/enhancement] Scale the vorticity hyperdiffusion coefficient with the acoustic-substepping outer step (`acoustic_substep_hyperdiffusion_scaling`). Under substepping the frozen hyperdiffusion filter is integrated at the outer step, so its explicit stability limit caps the outer-step multiple; `auto` (the default) reduces the coefficient to keep the filter within that limit, lifting the ceiling in proportion to `1/ν₄`. A number scales the coefficient directly, with `1` reproducing the unscaled coefficient. The scaling only affects substepping runs (`acoustic_substeps` nonzero); the default configuration and hyperdiffusion-off runs are unchanged. See `docs/src/acoustic_substepping.md`.
- [#4667](https://github.com/CliMA/ClimaAtmos.jl/pull/4667) ![][badge-✨feature/enhancement] Remove the acoustic-substepping resonance source and generalize the divergence damping. The kinetic-energy and reference-relative geopotential gradients are re-evaluated inside the sub-cycle by default (`acoustic_substep_kinetic_energy: fast`), removing the dominant source of the split-explicit resonance; the divergence damping gains a three-dimensional form (`acoustic_substep_damping_form: 3d`, default) and a perturbation form; and `acoustic_substep_damping: auto` resolves the coefficient from the kinetic-energy treatment. The previously validated configuration is recovered with `slow`/`horizontal`/`1.5`. See `docs/src/acoustic_substepping.md`.
